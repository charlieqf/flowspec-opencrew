...
```bash
python3 -m pytest \
  OpenCrew/backend/tests/contracts/test_koubo_storyboard_slot_state_contract.py \
  OpenCrew/backend/tests/contracts/test_koubo_storyboard_dialogue_asset_key_contract.py \
  OpenCrew/backend/tests/contracts/test_koubo_storyboard_manual_asset_status_contract.py \
  OpenCrew/backend/tests/contracts/test_koubo_storyboard_stale_edit_contract.py \
  tests/analysis_v1/test_storyboard.py \
  tests/analysis_v1/test_video_generation_plan.py \
  tests/analysis_v1/test_video_plan_executor.py \
  tests/analysis_v1/test_video_plan_composer.py
```

如果只改前端结构编辑、拖拽或播放，还要补：

```bash
node tests/storyboard/fixed_reorganize_timing_boundaries.mjs
node tests/storyboard/fixed_reorganize_task18_regression.mjs
```

如果改 UI 渲染、拖拽、保存、素材池 tab 或媒体播放，必须再做一次浏览器页面验证：打开 StoryBoard 详情页，执行拖拽 -> 保存 -> 刷新 -> 重进 Task -> 检查 JSON/文件/截图。测试音频时必须记录真实 `Audio.src`。
...
| M-22 | 两个 Node fixed reorganize 测试 | Shot/Scene/Dialogue 合并新增后的固定重排 |

### 4.3 手工页面最小回归

自动化跑完后，如果本次改动碰到前端/UI/绑定接口，至少执行以下 8 个页面案例：
...
| 验证项 | 命令/范围 | 结果 | 结论 |
| --- | --- | --- | --- |
| 槽位颜色案例表一致性 | `python3 OpenCrew/scripts/koubo_video_plan_slot_state_check.py --compare` | `differences=0` | 文档表与脚本派生规则一致 |
| 05-01 规划层 | `tests/analysis_v1/test_video_generation_plan.py` | `35 passed` | 口播/空镜/Segment/尾帧规划分支当前通过 |
| 后端合同测试 | Koubo StoryBoard slot/dialogue/manual/stale contracts，关闭 recording 插件后执行 | `17 passed, 4 failed` | 不能宣称后端合同全绿；失败项必须修复后才可作为强门禁 |
| 05-02/06-01 执行与合成 | `test_video_plan_executor.py`、`test_video_plan_composer.py`、`test_storyboard.py` | 当前存在失败 | 不能宣称执行/合成链路当前全绿 |

当前不能说“所有分支模块已经全量通过”。可以说的是：覆盖矩阵已经建立，规划层与槽位表一致性已经验证；后端合同、执行器、合成器还有失败项，必须修到全绿后，这套回归才具备“修改后产出等价”的门禁效力。

### 6.2 当前未通过案例索引

本节记录 2026-06-26 实跑时未通过、或暂时不能作为强门禁的案例。这里不代表测试案例无效；相反，它们是后续修状态机、修页面绑定、修执行链路时必须回看的失败索引。前端出现同类现象时，可以按“失败原因 / 预期结果”反查对应后端或工具链分支。

#### 6.2.1 后端合同测试未通过项

| 测试案例 | 当前失败原因 | 预期结果 |
| --- | --- | --- |
| `test_manual_dialogue_asset_key_overrides_plan_asset_key_for_all_plan_statuses` | `video_status["image"]["path"]` 是空字符串，手动 `dialogue_asset_key` 的素材状态没有压过旧 plan key | 返回 `SessionOutput/storyboard/Working/scene_001_dialogue_003_manual_Image_New.png`；手动 asset key 的新图状态优先于旧 plan key |
| `test_storyboard_video_slot_state_falls_back_to_dialogue_id_for_generated_raw_video` | `raw_video_exists=false`；手动 asset key 存在时，没有 fallback 到 dialogue_id 找生成 Raw 视频 | 即使手动 asset key 存在，也能识别 `scene_001_dialogue_003_Video_Raw.mp4`，Raw 槽位显示已存在 |
| `test_load_plan_ignores_legacy_edit_from_older_storyboard_tool_output` | `derive_dialogue_asset_key` 未定义，`normalize_source_plan` 直接 `NameError` | 旧编辑稿应被判定 stale 并忽略，使用最新 04-02 StoryBoard source normalize 出新 plan |
| `test_load_plan_keeps_edit_when_source_hash_matches_latest_storyboard_output` | 当前环境缺少 `opencc`，简繁转换初始化失败 | source hash 匹配时保留当前编辑稿，并正常完成 `load_plan` / `recalculate` |

#### 6.2.2 05-02 VideoPlanExecutor 未通过项

| 测试案例 | 当前失败原因 | 预期结果 |
| --- | --- | --- |
| `test_wan_rtv_duration_within_30_percent_tolerance_uses_max_10_seconds` | `provider_video_seconds(..., 4.0)` 返回 `4` | `wan2.7-r2v` 在 30% 容差规则下应使用最大 `10` 秒 |
| `test_gemini_image_generation_uses_nano_banana_v1beta_query_key_payload` | `image_gemini` 模块没有暴露 `common`，测试 mock 无法挂载 | Gemini 图片请求走统一 `common.post_json_request`，使用 nano-banana v1beta query key payload |
| `test_wan_rtv_generation_payload_matches_reference_image_aspect` | payload 第一项类型是 `reference_image` | WAN RTV payload 第一项应是 `first_frame`，第二项是 `reference_video`，并按参考图画幅设置尺寸 |
| `test_xai_video_does_not_force_mihomo_proxy` | `video_grok` 模块没有暴露 `common`，测试 mock 无法挂载 | xAI video 请求走统一 common，并验证不强制 mihomo proxy |
| `test_openai_gpt_image_generation_still_uses_openai_endpoints` | `image_gpt` 模块没有暴露 `common`，测试 mock 无法挂载 | GPT image 仍走 OpenAI multipart endpoint |
| `test_sync_lipsync_does_not_force_mihomo_proxy` | 当前调用 `requests_module.request`，测试 fake 只有 `post/get` | Sync.so create/status/download 流程完成，并验证不强制 mihomo proxy |
| `test_executes_segment_outputs_prompt_audio_image_video_lipsync` | 执行前置失败：`Dialogue audio task is missing dialogue_asset_key: srt_0001.`；provider mock 未进入 | 完成 image -> video -> lipsync -> tail，状态 `completed`，写回图片、视频和 StoryBoard 绑定 |
| `test_need_lipsync_false_retimes_video_and_replaces_audio` | 同样缺 `dialogue_asset_key`，非 lipsync 分支未进入 | 空镜/非 lipsync 完成 image -> video -> audio_video_sync -> tail，Final 视频为音频替换后结果 |
| `test_force_cleans_tool_dir_but_backs_up_storyboard_outputs` | 同样缺 `dialogue_asset_key`，force 分支未进入 | `--force` 先备份旧 Final 到 history，再生成新 Final，并清掉工具 Working stale 文件 |
| `test_bound_video_segment_materializes_without_video_model` | 同样缺 `dialogue_asset_key`，bound video 分支未进入 | 已绑定视频不跑视频模型，只走 audio_video_sync -> tail，`bound_video_completed_count=1` |
| `test_grok_talking_head_prompt_splits_speech_storyboard_and_scopes_pitfalls` | `speech_prompt` 缺少禁止字幕/屏幕文字的约束句 | 口播 Prompt 明确 `must never appear as subtitles or visible text`，并保持 Speech / Storyboard 分段 |
| `test_missing_default_model_config_blocks_without_fake_provider` | 当前先报缺 `dialogue_asset_key`，没有走到默认模型配置校验 | 缺默认图片模型时，优先报 `Default image model is not configured` |

#### 6.2.3 06-01 VideoPlanComposer 与 04-02 StoryBoard 未通过项

| 测试案例 | 当前失败原因 | 预期结果 |
| --- | --- | --- |
| `test_scene_compose_outputs_to_working_and_writes_storyboard` | 发布阶段找不到 `*_Scene_Subtitles.srt`，Scene 合成失败 | Scene 合成状态 `completed`，写入 Scene Final / Subtitled Final，并回写 StoryBoard |
| `test_shot_target_composes_scenes_before_shot` | 同样卡在第一个 Scene subtitles 文件缺失 | Shot 目标应先合成所有 Scene，再合成 Shot |
| `test_task_target_composes_shots_before_shot_plan` | 同样卡在 Scene subtitles 文件缺失 | Task 目标应先合成 Shots，再合成 ShotPlan |
| `test_compose_videos_maps_streams_by_type_for_concat` | ffmpeg filter graph 使用 `[0:v]`，不是 `[0:v:0]` | 按 stream 类型显式映射 `[0:v:0]` / `[0:a:0]`，避免音视频流顺序错位 |
| `test_hyperframe_failure_marks_scene_failed` | blocked reason 被缺 subtitles srt 覆盖，没有保留 mock 抛出的 `hyperframe failed` | HyperFrame 失败时 blocked reason 应保留真实异常 `hyperframe failed` |
| `test_storyboard_groups_srt_under_scene_without_merging_dialogue_records` | 模型输出校验失败：缺少 top-level `task_summary`，状态 `blocked` | 接受测试模型输出，生成 `analysis_v1_srt_storyboard_0.2`，一个 Scene 下保留两条独立 Dialogue |
