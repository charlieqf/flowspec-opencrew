...
StoryBoard 编辑页面加载完成后，工具归属必须遵守以下边界：

1. 只有“刷新 Session Variables”和 Video Prompt 编辑器中的“重新加载提示词”允许根据当前 workflow/profile 分流。
2. “刷新 Session Variables”负责选择对应 profile 的 `00_PrepareSessionVariables.py`；人物口播可以使用 `TalkingHead_V1/00`。
3. “重新加载提示词”只允许借用对应 profile 的提示词模块和模板重新构建 Prompt；人物口播 Max SD 2 可以使用 `TalkingHead_V1/05_02 + Video_SDR2V_TalkingHead.md`，但这不代表 StoryBoard 的视频计划执行器切换成 TalkingHead。
4. StoryBoard 内除上述两个入口外，其余计划生成、计划执行、状态读取、素材绑定、尾帧物化和合成均属于 `Analysis_V1`。其中 Video Plan 必须固定为 `Analysis_V1/05_01_VideoPlanGenerator.py -> Analysis_V1/05_02_VideoPlanExecutor.py`。
5. `workflow_id=person_talking_head_v1` 不得用于替换 StoryBoard 的 `05_02` 执行器。`TalkingHead_V1/05_02_VideoPlanExecutor.py` 只用于人物口播“一键成片”链路，以及“重新加载提示词”时作为提示词构建模块被局部加载。

已经踩过的故障：后端曾在 `video_plan_execution_script_path()` 中根据 `workflow_id` 把 StoryBoard Video Plan 从 Analysis 05_02 切到 TalkingHead 05_02。结果是 Analysis 05_01 生成的计划被另一套执行器消费，出现 `Lip-sync is required but disabled`，并在非首段回退到不匹配的 `Video_OpenRouter.md`，触发 `VIDEO_OPENROUTER_POSITIVE_BASE` 模板标记缺失。修复原则不是调整 lipsync 默认值，而是删除这条错误的执行器分流，并用合同测试锁定 StoryBoard 永远返回 Analysis 05_02。
...
3. 参考视频解析优先级为：Segment 显式 `provider_reference_video_path`、`source_face_masked_reference_video_path`、`reference_video_path`；三者均不存在时，使用固定兜底 `Reference/05_02/Video_SDR2V.mp4`，并复制到 `S9_05_02_VideoPlanExecutor/Working/Video_SDR2V.mp4`。
   - Tool Library 内的固定兜底素材时长必须小于等于 `15.0s`；当前标准文件为精确 `15.000s`。所有工具 Working 副本都必须从该标准文件重新复制，不得保留旧的超长副本。
4. 首帧/人物参考图与参考视频必须共同进入 OpenRouter 的 `input_references`。参考视频只提供参考人物的表情及表情变化、动作、姿态、节奏、手势和镜头运动，不得复制参考人物身份、脸、服装、身体、背景、水印或声音。
5. 空镜、产品特写、无人脸 Segment 继续使用通用 `Video_OpenRouter.md`，不得自动附加 `Video_SDR2V.mp4`。
6. DanceMimic 显式任务优先级高于本规则，继续使用 `Video_SDR2V_DanceMimic.md` 与其分段参考视频；Wan R2V、Kling Omni 和 TalkingHead_V1 的本地路由不受影响。
7. `prompt-only`、`video-only`、`prompt-and-video` 必须采用同一模型判断、模板选择和参考视频解析逻辑，并在模型调用审计中记录 `reference_mode=input_references`、`video_generation_mode=openrouter_sdr2v_talking_head` 与最终参考视频路径。

### 踩坑记录

- `video_openrouter.py` 的模块默认模板是通用 `Video_OpenRouter.md`；仅仅把默认模型设成 Max SD 2 不会自动切换模板，调用方必须把 `prompt_template=Video_SDR2V.md` 显式传入 `template_spec()`。
- 仅把 `Video_SDR2V.mp4` 复制到 Working 不等于模型已使用它；执行器还必须构造 `reference_videos`，最终由 `video_openrouter.py` 写入请求的 `input_references[].video_url`。
- Analysis_V1 与 TalkingHead_V1 的“重新加载提示词”来源不同：本链路使用 Analysis_V1 的 `Video_SDR2V.md`；TalkingHead_V1 使用 `Video_SDR2V_TalkingHead.md`，两者不得串用。

## 19. OpenRouter 参考视频的 R2 公网传输（2026-07-14）

1. 当运行环境提供完整的 `OPENCREW_PUBLIC_ASSET_R2_ENDPOINT`、`OPENCREW_PUBLIC_ASSET_R2_BUCKET`、`OPENCREW_PUBLIC_ASSET_R2_ACCESS_KEY_ID`、`OPENCREW_PUBLIC_ASSET_R2_SECRET_ACCESS_KEY` 时，`video_openrouter.py` 必须把本地参考视频上传到 R2，并把有时效的预签名 HTTPS URL 写入 `input_references[].video_url`；不得退回 tmpfiles。
2. `OPENCREW_PUBLIC_ASSET_R2_REGION`、`OPENCREW_PUBLIC_ASSET_R2_PREFIX`、`OPENCREW_PUBLIC_ASSET_R2_TTL_SECONDS` 作为可选运行时参数；非敏感的 provider、endpoint、bucket、region、prefix、TTL 必须注入本次 provider config，使模型调用审计能证明实际走的是 R2。
3. R2 Access Key 与 Secret 只允许在上传和签名时从运行环境读取，禁止复制到 `Variables.json`、Prompt、Result、ProviderTask、ModelCall 或任何审计 JSON。
4. 只有 R2 endpoint 或 bucket 不完整时才允许沿用原有发布器；provider config 显式指定非 R2 发布器时尊重显式配置，不由环境变量强制覆盖。
5. 修改 `~/.opencrew/public_assets_r2.env` 后必须重启后端进程，旧进程不会自动获得新增环境变量。

### R2 踩坑记录

- R2 已创建且凭证正确，不代表正在运行的后端已加载配置。若 ModelCall 中 `public_asset_provider` 为空，说明请求仍未进入 R2 分支。
- OpenRouter 的 `Invalid video_url` 通常表示提交的参考视频 URL 对模型侧不可访问；应先确认审计中的 provider 为 `r2`，再验证预签名 URL 可匿名 GET/Range 读取。
