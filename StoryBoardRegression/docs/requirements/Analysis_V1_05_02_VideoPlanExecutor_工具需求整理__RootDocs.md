...
7. 如果计划不要求对嘴型（空镜、产品特写、已绑定视频等），使用该 segment TTS 音频替换视频原音轨，并通过快放或慢放让视频时长以 TTS 音频为准。
8. 抽取最终视频尾帧。
9. 把最终图片、最终视频和必要业务审计文件同步回 `SessionOutput/storyboard/Working/`。
...
3. 视频最终落点：`SessionOutput/storyboard/Working/{first_dialogue_asset_key}_Video_Final.{ext}`，固定表示最终可交付视频；有人脸且 `need_lipsync=true` 时是对嘴后视频，无人脸、空镜、产品特写或已绑定视频时是完成音频替换与时长匹配后的最终视频。
4. 视频 Prompt 落点：`SessionOutput/storyboard/Working/{first_dialogue_asset_key}_VideoPrompt.json`
5. 尾帧最终落点：`SessionOutput/storyboard/Working/{first_dialogue_asset_key}_TailFrame.{ext}`
...
1. 对嘴前 raw video、对嘴临时视频、绑定视频拷贝、音频替换临时视频、模型下载临时文件、探测临时文件都只能放在 `S9_05_02_VideoPlanExecutor/Working/`。
...
4. 后续 segment 链式生成使用的尾帧应来自最终 `Video_Final`；如果 `need_lipsync=true` 且对嘴失败，默认不使用 raw video 尾帧继续链式生成。
5. 本规则限定图片/视频媒体文件；音频最终文件和业务 Prompt JSON 按本工具各自合同保存，模型中间响应不进入 StoryBoard Working。
...
SessionOutput/storyboard/video_generation_plan.json
OpenCrew/ToolLibrary/Analysis_V1/Reference/05_02/*.md
```
...
S9_05_02_VideoPlanExecutor/Prompt/Ref_05_02_Image_GPT.md
S9_05_02_VideoPlanExecutor/Prompt/Ref_05_02_Image_Gemini.md
S9_05_02_VideoPlanExecutor/Prompt/Ref_05_02_Image_Grok.md
S9_05_02_VideoPlanExecutor/Prompt/Ref_05_02_Video_GPT.md
S9_05_02_VideoPlanExecutor/Prompt/Ref_05_02_Video_Gemini.md
S9_05_02_VideoPlanExecutor/Prompt/Ref_05_02_Video_Grok.md
S9_05_02_VideoPlanExecutor/Prompt/Ref_05_02_Video_Wan.md
S9_05_02_VideoPlanExecutor/Prompt/Ref_05_02_Lipsync_SyncSo.md
S9_05_02_VideoPlanExecutor/Prompt/Ref_05_02_Lipsync_HeyGen.md
```
...
### 4.5.1 图像 / 视频 / 口型同步模块化边界

`05_02_VideoPlanExecutor.py` 保持唯一工具入口，但图像、视频、口型同步必须拆成内部子模块。子模块不是 ToolLibrary 工具，不进入 `tool_registry.json`，不能被前端或工具链当成独立步骤调度。

推荐实现目录：

```text
OpenCrew/ToolLibrary/Analysis_V1/video_plan_executor_modules/
  image_gpt.py
  image_gemini.py
  image_grok.py
  video_gpt.py
...
1. 读取 `Variables.json`、`srt_storyboard.json`、`video_generation_plan.json`。
2. 判断当前 segment 是否需要 image / video / lipsync。
3. 根据 provider/model 选择内部子模块。
4. 把 segment、shot、scene、dialogue index、reference paths、provider config 等上下文传给子模块。
5. 接收子模块返回的 Prompt 文件、模型响应和产物路径。
6. 统一写 execution state / result、发布文件、备份覆盖、回写 StoryBoard。

主工具不允许负责：

1. 拼接图片 Prompt。
2. 拼接视频 Prompt。
3. 拼接口型同步 Prompt 或 Sync 请求说明。
4. 为不同模型写通用 Prompt 风格。
5. 在模型调用时临场重新拼接 Prompt。

每个图像 / 视频 / 口型同步子模块必须同时负责：

1. 读取自己的 Reference 模板快照。
2. 从主工具传入的上下文里提取本模型需要的字段。
3. 按本模型自己的语言风格和限制拼接 Prompt package。
4. 写出可审计的 `PromptRendered_*` JSON。
5. 支持 dry-run / prompt-only 测试模板修改，不调用真实模型。
6. 调用模型时只读取已经落盘的 Prompt JSON，不重新拼接隐藏 Prompt。
7. 记录 provider/model/endpoint/输入文件/输出文件/非敏感参数，且不写真实 API key。

子模块内部也不应把不同模型的提示词读取和拼接方式抽成统一 `common.py` Prompt composer。`common.py` 只保留 JSON、文件、HTTP、字段抽取等底层能力；每个模型模块必须保留自己的模板读取、块选择、变量替换和拼接方式。Python 模块不得保存正向 / 负向提示词正文，提示词正文必须在对应 `Reference/05_02/*.md` 模板中维护。

统一模块接口建议：

```python
def build_prompt_package(context: dict) -> dict:
    ...

def write_prompt_package(prompt_dir: Path, asset_key: str, package: dict) -> Path:
    ...

def dry_run_prompt(context: dict, prompt_dir: Path, asset_key: str) -> dict:
    ...

def generate(context: dict, prompt_path: Path, output_path: Path) -> dict:
    ...
```
...
OpenCrew/ToolLibrary/Analysis_V1/Reference/05_02/Lipsync_SyncSo.md
OpenCrew/ToolLibrary/Analysis_V1/Reference/05_02/Lipsync_HeyGen.md
```

运行开始时，主工具只负责把这些 Reference 文件复制为 Prompt 快照，例如：
...
随后由对应子模块读取这些快照并生成 Prompt package。这样修改 `Image_Gemini.md` 后，可以只 dry-run `image_gemini.py` 的 Prompt 结果；修改 `Video_Grok.md` 后，可以只 dry-run `video_grok.py` 的 Prompt 结果，不需要触碰主工具和其它模型。
...
S9_05_02_VideoPlanExecutor/Working/srt_0001_01_PreviousTailFrame.jpg
```
...
`05_01_VideoPlanGenerator.py` 需要在每个 video segment 中写入 Sync 控制字段：
...
1. 默认口播人物段 `need_lipsync=true`、`sync_mode=lipsync`。
2. 如果前端明确关闭该 video segment 的对嘴型，`need_lipsync=false` 且 `lipsync_disabled_by_ui=true`，但仍必须 `need_audio_video_sync=true`、`sync_mode=audio_replace_retime`。
3. 如果计划或 StoryBoard 明确标记无人脸 / 产品特写 / 不需要口型，`need_lipsync=false`，并执行音频替换与时长匹配 Sync。
...
运行开始时，必须只把模块级参考模板复制到 `Prompt/`。拆分后不再复制外层大提示词模板：
...
本工具运行过程中，图像 / 视频 / 口型同步子模块只能读取这个 `Prompt/` 目录内的模板快照，不应直接读取源模板文件。主工具不拼接这些 Prompt，只复制模板快照并选择模块。
...
3. 执行 Dialogue 音频任务。
4. 按 segment 内 dialogue 顺序合成 `SegmentAudio_Final`，作为视频对嘴型或音频替换输入；如果 segment 只有一个 Dialogue，也必须生成或确认 segment 级音频记录。
...
15. 如果 `need_lipsync=false`，不调用对嘴型模型；对 raw video 或绑定视频拷贝执行音频替换与时长匹配，让最终视频时长以 Segment TTS 音频为准。
16. 将最终视频写入 `Output/`。
17. 覆盖 StoryBoard Working 中同名文件前，按 History 规则备份旧文件。
18. 将最终视频从 `Output/` 复制到 `SessionOutput/storyboard/Working/`。
19. 从最终视频抽取尾帧，先写入 `Output/`。
20. 将最终尾帧从 `Output/` 复制到 `SessionOutput/storyboard/Working/`。
...
6. 如果 `need_lipsync=false`，raw video 校验通过后必须先完成音频替换与时长匹配；只有 sync video 可以晋升为最终视频，不视为中间视频泄漏。
...
```json
{
  ...
  "steps": {
    ...
    "sync": {
      ...
      "mode": "audio_replace_retime",
      "output_path": "S9_05_02_VideoPlanExecutor/Working/srt_0001_01_Video_AudioSynced.mp4"
    }
  }
}
```
...
3. `completed_working`：对空镜、产品特写和绑定视频也表示 Sync 已完成，此时完成的是音频替换与时长匹配，而不是对嘴型模型调用。
...
1. `05_02` 不调用对嘴型模型。
2. raw video 校验通过后必须进入音频替换与时长匹配。
3. Sync step 记录为 `completed_working`，输出为替换音轨后的最终视频。
4. 该完成状态不算失败，也不阻断后续 tail frame 依赖。
...
10. 执行非对嘴 Sync：`need_lipsync=false` 时，用 Segment 音频替换 raw video 或绑定视频的原音轨，并按音频时长快放或慢放视频。
11. 生成最终视频：`need_lipsync=true` 时对嘴后视频成为 `{first_dialogue_asset_key}_Video_Final.{ext}`；`need_lipsync=false` 时音频替换与时长匹配后的 sync video 成为 `{first_dialogue_asset_key}_Video_Final.{ext}`。
12. 抽取最终尾帧：从最终视频抽取尾帧，供后续链式 segment 使用。
...
{dialogue_asset_key}_PreviousTailFrame.{ext}
...
```text
Ref_05_02_Image_GPT.md
Ref_05_02_Image_Gemini.md
Ref_05_02_Image_Grok.md
Ref_05_02_Video_GPT.md
Ref_05_02_Video_Gemini.md
Ref_05_02_Video_Grok.md
Ref_05_02_Video_Wan.md
Ref_05_02_Lipsync_SyncSo.md
Ref_05_02_Lipsync_HeyGen.md
...
{first_dialogue_asset_key}_TailFrame.{ext}
```
...
SessionOutput/storyboard/Working/{first_dialogue_asset_key}_TailFrame.{ext}
```
  ...
  "segments": [
    {
      ...
      "tool_outputs": {
        ...
        "tail_frame_path": "S9_05_02_VideoPlanExecutor/Output/srt_0001_01_TailFrame.jpg"
      },
      "storyboard_outputs": {
        ...
        "tail_frame_path": "SessionOutput/storyboard/Working/srt_0001_01_TailFrame.jpg"
      },
      "blocked_reason": "",
      "failed_reason": ""
    }
...
1. 生成音频后，写入对应 Dialogue `working_assets.audio.path`。
2. 生成图片后，写入对应 Dialogue `working_assets.images[0].path`。
3. 生成视频后，写入对应 Dialogue `working_assets.video.path`。
...
2. 四个目录内部均无子目录。
3. 模块模板文档复制到 `Prompt/Ref_05_02_{Module}.md`，不再复制外层大提示词模板。
...
12. 最终视频先进入 Output，再进入 `SessionOutput/storyboard/Working/`；`need_lipsync=true` 时是对嘴后视频，`need_lipsync=false` 时是替换 TTS 音轨并按音频时长变速后的最终视频。
...
9. `SessionOutput/storyboard/Working/{first_dialogue_asset_key}_TailFrame.{ext}`
...
| 是否复制模板到 Prompt | 是，只复制 `Reference/05_02/` 下的模块模板到 Prompt |
...
| 最终视频产物先到哪里 | 最终视频先到工具 Output，再同步到 StoryBoard Working；有人脸且 `need_lipsync=true` 时是对嘴后视频，无人脸、空镜、产品特写或绑定视频时是替换 TTS 音轨并按音频时长变速后的最终视频 |
