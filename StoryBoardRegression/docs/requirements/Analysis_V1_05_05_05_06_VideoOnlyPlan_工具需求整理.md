...
1. `05_06` 必须复用 `05_02` 的模型选择、模板选择和参考视频准备函数；模型只来自 `Variables.default_video_config`。
2. `openrouter / bytedance/seedance-2.0` 的非空镜、非 DanceMimic Segment 使用 `Video_SDR2V.md`；显式参考视频优先，否则使用 `Video_SDR2V.mp4`。
   - 固定兜底视频必须小于等于 `15.0s`，并由时长契约测试保护。
3. 首帧图片和参考视频必须共同进入 OpenRouter `input_references`。只复制兜底视频而没有传 `reference_videos` 属于未接通。
4. 空镜仍用 `Video_OpenRouter.md` 且不注入 SDR2V 视频；DanceMimic 继续使用自己的模板与分段参考视频。
5. Prompt、Video、Prompt + Video 三条路径必须一致，并在审计中记录最终参考视频与参考模式。
6. TalkingHead Max SD 2 的尾帧物化必须从 StoryBoard 根部恢复隐私配置，调用本地连续帧红网格函数后再发布下一段 `Image_New`；后端物化接口和 `05_06` 执行路径都必须覆盖。
...
- 不得从 TalkingHead 数据库字段推断 Analysis_V1 模型；数据库只补密钥与连接参数。
- `build_video_prompt()` 必须显式传递 `video_selection.prompt_template`。
- Analysis_V1 与 TalkingHead_V1 的模板文件不同，重新加载提示词时不得串用。
...
## 20. 05_06 OpenRouter 参考视频 R2 传输回归要求（2026-07-14）

- `05_06` 复用 `05_02 -> video_openrouter.py` 的 R2 解析；完整配置存在时，显式或兜底参考视频均通过 R2 预签名 URL 进入 OpenRouter。
- 审计只记录非敏感传输字段，Access Key 与 Secret 禁止落入 Variables、Prompt、Result、ProviderTask 或 ModelCall。
- 更新 R2 环境配置后必须重启后端；回归需验证 provider 为 `r2`、URL 可 GET/Range，且完整 R2 配置下不出现 tmpfiles。
