...
6. `bound_video`：来自 Dialogue 已绑定的视频素材。它不是首帧图片，而是“该 Dialogue 已有人为落位视频”的完成态视觉来源；它只能代表当前 Segment 的第一个 Dialogue，不能覆盖后续 Dialogue。
7. `missing`：没有可用首帧或已绑定视频，不能生成视频。

### 6.2.1 统一 Segment Truth

`Video Plan` / `Video Only Plan` / `Image Plan` 必须复用同一套 Segment 拆分真相：

1. Segment 边界只能由 `05_01_VideoPlanGenerator.py` 的统一拆分逻辑决定。
2. Plan 类型只能决定每个 Segment 上显示和执行哪些任务，不能重新决定 Segment 包含哪些 Dialogue。
3. `bound_video` 只改变当前 Segment 的任务状态：该 Segment 不需要重新生成 Raw Video，但仍需要按 Video Plan 规则做音频替换 / 时长同步 / 尾帧抽取。
4. `bound_video` 不能跳过 `max_video_seconds` / `split_tolerance_seconds`，也不能把后续无视觉锚点 Dialogue 合并进同一个已有终视频 Segment。
5. 如果某个 Dialogue 已绑定 Final Video，正确拆分是：该 Dialogue 单独成为 `bound_video` Segment；后续 Dialogue 如果没有自己的视觉锚点，应从该 Segment 的 TailFrame 继续，并按普通时长规则继续拆分。
6. `Video Only Plan` 生成 source plan 时不得为了生成 Raw 任务而清空 StoryBoard 中的 Final Video 绑定；Final / Raw / Prompt 只影响任务状态，不影响 Segment 边界。
...
4. 绑定视频表示该 Segment 的视频生成步骤已经有人为落位结果，但仍必须执行音频替换与时长匹配 Sync；计划中必须标记为待执行态：
...
5. `05_01` 只写计划，不拷贝视频文件。`05_02` 执行时必须先把绑定视频拷贝到工具 `Working/`，再用 Segment TTS 音频替换原视频音轨，并通过变速让视频时长匹配 Segment 音频时长，最终发布到 `planned_outputs.video_path`，命名规则仍为 `{first_dialogue_asset_key}_Video_Final.mp4`。
6. 如果绑定视频已经在 `SessionOutput/storyboard/Working/` 中，但文件名不是该 Segment 的规范命名，`05_02` 仍需拷贝或重命名到规范路径；不能直接把非规范路径当作当前 Segment 完成产物。
7. 绑定视频完成的是视频生成步骤。Dialogue 音频仍按 `dialogue_audio_tasks[]` 独立判断；如果后续合成需要 Segment 级音频，仍由 `05_02` 根据 plan 处理。
8. 绑定视频 Segment 不执行对嘴型模型，但必须执行 `audio_replace_retime` Sync。是否允许后续空 Dialogue 继承它的尾帧，由该 Segment 首个 Dialogue 的 `video_plan.is_talking_head` 决定：`false` 表示空镜，尾帧不可继承；`true` 或字段缺失按兼容策略视为口播尾帧可继承。
          ...
          "segments": [
            {
              ...
              "tail_frame": {
                "planned_path": "SessionOutput/storyboard/Working/srt_0001_01_TailFrame.jpg",
                "available": false,
...
7. 绑定视频 Segment 不得包含后续 Dialogue；它的后续 Segment 通过 `previous_segment_tail_frame` 依赖该绑定视频 Segment 的 TailFrame。
...
1. 如果图片 / 尾帧驱动生成的 Segment 首个 Dialogue 存在 `video_plan.is_talking_head=false`，该 Segment 不规划对嘴型，但仍规划音频替换与时长匹配 Sync：
...
4. 如果 Segment 来源是绑定视频，`05_01` 不规划对嘴型，原因记录为 `existing_video_bound_complete`；同时必须规划 `need_audio_video_sync=true` 和 `sync_mode=audio_replace_retime`，用于后续替换原视频音轨并按 TTS 音频时长变速视频。该 Segment 是否允许后续空 Dialogue 使用尾帧，仍读取同一个 `video_plan.is_talking_head` 字段：`false` 表示空镜尾帧不可继承，`true` 或缺失表示按口播尾帧兼容处理。
...
{first_dialogue_asset_key}_TailFrame.jpg
```
...
27. 绑定视频 Segment 的视频生成步骤必须是完成态：`need_video_prompt=false`、`need_video=false`、`planned_outputs.video_path` 为规范 Working 路径；Sync 仍需执行 `audio_replace_retime`。
28. 绑定视频 Segment 必须记录 `existing_video.materialize_video.copy_from_path` 和 `copy_to_path`，供 `05_02` 拷贝到工具 Working 后替换 TTS 音轨并按音频时长变速。
...
| Dialogue 已绑定视频 | 该 Dialogue 作为新的 Segment 起点；该 Segment 视频生成步骤完成，`05_02` 先拷贝绑定视频，再替换 TTS 音轨并按音频时长变速到规范 Working 视频路径。 |
...
| 是否按照 Rerun 和断点继续实现？ | 是。`--force` 清理并重建本工具目录和本工具拥有的 `SessionOutput/storyboard/video_generation_plan.json`；`--resume` 在 StoryBoard 输入 hash、参数 hash 一致且 Output 可信时复用。 |
| 原始状态是什么？ | `S8_05_01_VideoPlanGenerator/` 不存在或为空，且 `SessionOutput/storyboard/video_generation_plan.json` 不存在。 |
| 强制 Rerun 如何恢复原始状态？ | 只清理 `S8_05_01_VideoPlanGenerator/Working`、`Output`、`Report`，以及本工具拥有的 `SessionOutput/storyboard/video_generation_plan.json`，不删除上游 StoryBoard、不删除素材、不改 `SessionContext`。 |
...
21. 绑定视频 Dialogue 作为新的 Segment 起点，且视频生成步骤完成；执行时由 `05_02` 拷贝到工具 Working，再完成 TTS 音轨替换和时长匹配后发布到规范 Working 视频路径。
...
1. 绑定视频不由 `05_01` 管视频时长；`05_02` 在 Sync 阶段按 Segment TTS 音频时长执行变速和音轨替换。
2. 绑定视频只代表视频步骤完成，不代表音频步骤完成；Dialogue 音频仍按 `dialogue_audio_tasks[]` 判断。
3. 绑定视频必须由 `05_02` 拷贝到规范 Working 路径，不能直接把上传源路径当最终视频。
4. 绑定视频尾帧必须从完成音频替换和时长匹配后的规范 Working 视频提取。
5. 空镜只阻断后续“无视觉来源续接”；后续 Dialogue 如果自己有原图 / 新图 / 绑定视频，仍可正常开启新 Segment。
6. 旧数据没有 `dialogue.video_plan.is_talking_head` 时，默认按口播处理，不默认 blocked。
7. Segment 不跨 Scene，但尾帧依赖可以跨 Scene；跨 Scene 续接也必须遵守空镜尾帧不可继承。

最终实现原则：`05_01` 只做规划和阻断判断；`05_02` 只按计划执行拷贝 / 生成 / 提取尾帧，不重新推断 Segment 边界，也不绕过空镜阻断。

### 18.1 绑定视频时长不参与 05-01 判断，但由 05-02 Sync 适配

绑定视频 Segment 的时间范围仍由被覆盖的 Dialogue 时间决定。绑定视频本身可能长于或短于 Dialogue 时间：

1. `05_01` 不裁剪、不拉伸，只记录 `duration`、`planned_video_duration` 和绑定视频路径。
2. `05_02` 不因为绑定视频时长不匹配而回退到重新生成视频；它必须在 Sync 阶段探测视频和 Segment 音频时长，用快速或慢速变速视频，让最终视频时长以 TTS 音频为准。
3. Sync 后的最终视频替换原视频音轨，只保留 Segment TTS 音频。
4. 只要绑定视频文件存在且可拷贝，Video 步骤即可完成；但最终 `Video_Final` 只有在音频替换与时长匹配完成后才算完成。
