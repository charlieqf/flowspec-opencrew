...
1. 规范业务文件已经真实存在且非空时，优先显示绿色；历史 execution state 中的 failed / running 不能覆盖已落盘事实。
2. 只有当前执行任务仍在运行，且该步骤对应规范业务文件尚未落盘时，才显示黄色运行态。
3. 只有当前步骤没有可用规范业务文件，并且当前 execution state 明确失败时，才显示红色错误态。
4. 白色只表示“当前条件满足，可以执行但尚未落盘”；灰色只表示“不执行 / 跳过 / blocked”。
5. 如果文件存在但 StoryBoard JSON 绑定缺失，媒体文件本身对应步骤可以显示“文件已落盘”，但最终槽位完成态不能显示绿色成功，必须提示需要补绑定或刷新。
