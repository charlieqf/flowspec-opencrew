...
| Dialogue `Video_Final` | 视频 | `dialogue.working_assets.video.path` + `source_type` | 图片、声音、无效文件 | 清空该 Dialogue 的视频槽；`generated` 进 history，`upload` 回上传素材池 |
...
| DR-06 | 上传素材 | 视频 | Dialogue `Video_Final` | `working_assets.video.path` 指向 `assets/videos/...`，`source_type = upload`，不复制到 `Working/` |
...
| DR-09 | 历史素材 | 视频 | Dialogue `Video_Final` | 恢复为当前素材时必须复制回 `Working/`，并写为 `source_type = generated` |
...
| RP-03 | Dialogue `Video_Final` 从视频 A 替换为视频 B | 只更新该 Dialogue 的 `working_assets.video.path`；旧 `generated` 视频进 history，旧 `upload` 视频回上传素材池 |
