...
TOOL_ID = "08_01_Scene_PlanA_ImageSequenceClipPlan"
TOOL_NAME = "08_01_Scene_PlanA_ImageSequenceClipPlan"
TOOL_VERSION = "1.0.0"
TOOL_LEVEL = "scene"
REQUIRES = ['rebuild_shot_plan.json', 'scene_assets', 'locked_tts', 'shot_srt']
PRODUCES = ["reports/plan_a/08_01_Scene_PlanA_ImageSequenceClipPlan.json"]
SUGGESTED_PREVIOUS_TOOLS = ['06_02_Scene_PlanA_SceneImageSelect', '07_03_Shot_PlanA_TTSTimelineValidate']
SUGGESTED_NEXT_TOOLS = ['08_02_Shot_PlanA_HyperframeSubtitleAlign']
...
def image_sequence_clip_plan(workspace: Path, plan: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    results, blockers = [], []
    for shot in target_shots(plan, args):
        shot_id = shot_id_of(shot); timeline_path = canonical_shot_tts_timeline_path(workspace, shot_id); timeline = read_json(timeline_path) if timeline_path.exists() else {}; pages = [item for item in timeline.get("image_pages") or [] if isinstance(item, dict)]
        for mark in target_scene_marks(shot, args):
            scene_id = scene_id_of(mark); missing = []; image = canonical_scene_image_path(workspace, shot_id, scene_id); clip_path = variant_scene_dir(workspace, shot_id, scene_id) / "plan_a_clip_plan.json"
            for name, path in (("missing_locked_tts", canonical_locked_tts_path(workspace, shot_id)), ("missing_shot_srt", canonical_shot_srt_path(workspace, shot_id)), ("missing_shot_tts_timeline", timeline_path), ("missing_scene_first_image", image)):
                if not path.exists() or (path.is_file() and path.stat().st_size <= 0): missing.append(name)
            page = next((item for item in pages if str(item.get("scene_mark_id") or "") == scene_id), {})
            if missing:
                blockers.extend(f"{shot_id}/{scene_id}: {item}" for item in missing); results.append({"shot_id": shot_id, "scene_mark_id": scene_id, "status": "blocked", "blocking_errors": missing, "clip_plan": rel(workspace, clip_path)}); continue
            clip = {"shot_id": shot_id, "scene_mark_id": scene_id, "status": "planned", "image": rel(workspace, image), "audio": rel(workspace, canonical_locked_tts_path(workspace, shot_id)), "shot_srt": rel(workspace, canonical_shot_srt_path(workspace, shot_id)), "start": safe_float(page.get("start"), 0.0), "end": safe_float(page.get("end"), safe_float(timeline.get("duration"), safe_float(shot.get("duration"), 1.0))), "duration": safe_float(page.get("duration"), 1.0), "text": scene_text(mark, spoken_script(shot)), "generated_at": now_ms()}
            write_json(clip_path, clip); results.append({**clip, "clip_plan": rel(workspace, clip_path)})
    path = variant_shot_dir(workspace, shot_id) / "reports" / "plan_a_08_01_image_sequence_clip_plan.json"; payload = {"status": "completed_with_blockers" if blockers else "completed", "tool_id": TOOL_ID, "blocking_errors": blockers, "item_count": len(results), "results": results}; write_json(path, payload); return {**payload, "output": rel(workspace, path)}

def run_tool(workspace: Path, plan: dict[str, Any], source_package: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    return image_sequence_clip_plan(workspace, plan, args)
