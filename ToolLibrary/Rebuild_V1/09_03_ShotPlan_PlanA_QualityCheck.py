...
TOOL_ID = "09_03_ShotPlan_PlanA_QualityCheck"
TOOL_NAME = "09_03_ShotPlan_PlanA_QualityCheck"
TOOL_VERSION = "1.0.0"
TOOL_LEVEL = "shotplan"
REQUIRES = ['rebuild_shot_plan.json']
PRODUCES = ["reports/plan_a/09_03_ShotPlan_PlanA_QualityCheck.json"]
SUGGESTED_PREVIOUS_TOOLS = ['09_02_ShotPlan_PlanA_Compose']
...
def quality_check(workspace: Path, plan: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    rows, blockers = [], []
    total_shot_duration = 0.0
    total_audio_duration = 0.0
    for shot in shot_list(plan):
        shot_id = shot_id_of(shot); video = plan_a_shot_video_path(workspace, shot_id); locked = canonical_locked_tts_path(workspace, shot_id); warnings = []
        video_duration = ffprobe_duration(video) if video.exists() and video.stat().st_size > 0 else None
        audio_duration = ffprobe_duration(locked) if locked.exists() and locked.stat().st_size > 0 else None
        if not video.exists() or video.stat().st_size <= 0: warnings.append("missing_plan_a_video")
        elif not video_duration: warnings.append("unreadable_plan_a_video")
        if not locked.exists() or locked.stat().st_size <= 0: warnings.append("missing_locked_tts")
        elif not audio_duration: warnings.append("unreadable_locked_tts")
        if video_duration: total_shot_duration += video_duration
        if audio_duration: total_audio_duration += audio_duration
        if video_duration and audio_duration and abs(video_duration - audio_duration) > 1.0: warnings.append("shot_video_audio_duration_drift")
        blockers.extend(f"{shot_id}: {item}" for item in warnings); rows.append({"shot_id": shot_id, "plan_a_video_path": rel(workspace, video) if video.exists() else "", "locked_tts_path": rel(workspace, locked) if locked.exists() else "", "video_duration": video_duration, "audio_duration": audio_duration, "warnings": warnings})
    final = workspace / "renders" / "plan_a.mp4"
    final_duration = ffprobe_duration(final) if final.exists() and final.stat().st_size > 0 else None
    if not final.exists() or final.stat().st_size <= 0: blockers.append("shot_plan: missing renders/plan_a.mp4")
    elif not final_duration: blockers.append("shot_plan: unreadable renders/plan_a.mp4")
    elif total_shot_duration and abs(final_duration - total_shot_duration) > 1.0: blockers.append("shot_plan: final_duration_drift_from_shots")
    payload = {"status": "completed_with_blockers" if blockers else "completed", "tool_id": TOOL_ID, "blocking_errors": blockers, "final_output": rel(workspace, final), "final_duration": final_duration, "total_shot_duration": round(total_shot_duration, 3), "total_audio_duration": round(total_audio_duration, 3), "shot_count": len(rows), "shots": rows}; write_json(workspace / "reports" / "plan_a" / f"{TOOL_ID}.json", payload); return payload

def run_tool(workspace: Path, plan: dict[str, Any], source_package: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    return quality_check(workspace, plan, args)
