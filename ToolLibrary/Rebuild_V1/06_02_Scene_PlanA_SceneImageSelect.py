...
TOOL_ID = "06_02_Scene_PlanA_SceneImageSelect"
TOOL_NAME = "06_02_Scene_PlanA_SceneImageSelect"
TOOL_VERSION = "1.0.0"
TOOL_LEVEL = "scene"
REQUIRES = ['rebuild_shot_plan.json', 'scene_first_images']
PRODUCES = ["reports/plan_a/06_02_Scene_PlanA_SceneImageSelect.json"]
SUGGESTED_PREVIOUS_TOOLS = ['06_01_Scene_PlanA_SceneImageRebuild']
SUGGESTED_NEXT_TOOLS = ['08_01_Scene_PlanA_ImageSequenceClipPlan']
...
def scene_image_select(workspace: Path, plan: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    items, blockers = [], []
    for shot in target_shots(plan, args):
        shot_id = shot_id_of(shot)
        for mark in target_scene_marks(shot, args):
            scene_id = scene_id_of(mark); image_path = canonical_scene_image_path(workspace, shot_id, scene_id); manifest_path = scene_asset_manifest_path(workspace, shot_id, scene_id); row = {"shot_id": shot_id, "scene_mark_id": scene_id, "selected_image": rel(workspace, image_path), "manifest": rel(workspace, manifest_path)}
            if not image_path.exists(): blockers.append(f"{shot_id}/{scene_id}: missing_scene_first_image_asset"); items.append({**row, "status": "blocked", "blocking_reason": "missing_scene_first_image_asset"}); continue
            if not manifest_path.exists(): write_json(manifest_path, {**row, "status": "completed", "source": "selected_existing_image_without_manifest", "generated_at": now_ms()})
            mark.setdefault("plan_a", {})["scene_asset"] = {"selected_image": rel(workspace, image_path), "manifest": rel(workspace, manifest_path), "selected_at": now_ms()}; items.append({**row, "status": "selected"})
    path = variant_shot_dir(workspace, shot_id) / "reports" / "plan_a_06_02_scene_image_select.json"; payload = {"status": "completed_with_blockers" if blockers else "completed", "tool_id": TOOL_ID, "blocking_errors": blockers, "item_count": len(items), "items": items}; write_json(path, payload); return {**payload, "output": rel(workspace, path)}

def run_tool(workspace: Path, plan: dict[str, Any], source_package: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    return scene_image_select(workspace, plan, args)
