...
TOOL_ID = "07_03_Shot_PlanA_TTSTimelineValidate"
TOOL_NAME = "07_03_Shot_PlanA_TTSTimelineValidate"
TOOL_VERSION = "1.0.0"
TOOL_LEVEL = "shot"
REQUIRES = ['rebuild_shot_plan.json', 'locked_tts', 'shot_srt', 'shot_tts_timeline']
PRODUCES = ["reports/plan_a/07_03_Shot_PlanA_TTSTimelineValidate.json"]
SUGGESTED_PREVIOUS_TOOLS = ['07_02_Shot_PlanA_TTSGenerateAndLock']
SUGGESTED_NEXT_TOOLS = ['08_01_Shot_PlanA_ImageSequenceClipPlan']
...
def tts_timeline_validate(workspace: Path, plan: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    results, blockers = [], []
    for shot in target_shots(plan, args):
        shot_id = shot_id_of(shot); missing = []; warnings = []; locked = canonical_locked_tts_path(workspace, shot_id); srt_path = canonical_shot_srt_path(workspace, shot_id); timeline_path = canonical_shot_tts_timeline_path(workspace, shot_id)
        if not locked.exists(): missing.append("missing_locked_tts")
        elif locked.stat().st_size <= 0: missing.append("empty_locked_tts")
        if not srt_path.exists(): missing.append("missing_shot_srt")
        ...
        srt_count = 0
        if audio_duration is None and locked.exists() and locked.stat().st_size > 0:
            missing.append("unreadable_locked_tts_duration")
        if srt_path.exists():
            srt_count = len(parse_srt_entries(srt_path.read_text(encoding="utf-8")))
            if srt_count == 0:
                warnings.append("shot_srt_has_no_timed_entries")
        if timeline_path.exists():
            timeline = read_json(timeline_path)
            timeline_duration = safe_float(timeline.get("duration"), 0.0) if isinstance(timeline, dict) else 0.0
            if timeline_duration <= 0:
                missing.append("invalid_tts_timeline_duration")
            if audio_duration and timeline_duration and abs(audio_duration - timeline_duration) > 0.75:
                warnings.append("audio_timeline_duration_drift")
        blockers.extend(f"{shot_id}: {item}" for item in missing); results.append({"shot_id": shot_id, "status": "validated" if not missing else "blocked", "blocking_errors": missing, "warnings": warnings, "audio_duration": audio_duration, "timeline_duration": timeline_duration, "srt_entry_count": srt_count})
...
def run_tool(workspace: Path, plan: dict[str, Any], source_package: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    return tts_timeline_validate(workspace, plan, args)
