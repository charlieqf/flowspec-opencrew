...
TOOL_ID = "04_02_Shot_FirstLastFrameConfirm"
TOOL_NAME = "Shot First Last Frame Confirm"
TOOL_VERSION = "1.0.0"
REQUIRES = ["rebuild_shot_plan.json", "marked_first_last", "shot_id"]
PRODUCES = ["rebuild_shot_plan.json", f"reports/rebuild_v1/{TOOL_ID}.json"]
SUGGESTED_PREVIOUS_TOOLS = ["04_01_Shot_FirstLastFrameMark", "04_01_ShotPlan_FirstLastFrameMark"]
...
def run(workspace: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    results = [confirm_scene(shot, mark) for mark in scene_marks_for_shot(shot)]
    ...
    if not plan_path.exists():
        ...
        if len(args.shot_id) == 1:
            try:
                ...
                if not shot:
                    missing.append({"dependency": "shot_id", "reason": "target shot was not found", "suggested_tools": SUGGESTED_PREVIOUS_TOOLS, "scope": scope(args)})
                elif not scene_marks_for_shot(shot):
                    missing.append({"dependency": "scene_marks", "reason": "target shot has no scene marks", "suggested_tools": ["04_01_Shot_FirstLastFrameMark"], "scope": scope(args)})
                elif all(can_confirm(mark)[0] for mark in scene_marks_for_shot(shot)):
