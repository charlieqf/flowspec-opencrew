...
TOOL_NAME = "06_01_Scene_PlanA_SceneImageRebuild"
TOOL_VERSION = "1.0.0"
TOOL_LEVEL = "scene"
REQUIRES = ['rebuild_shot_plan.json', 'confirmed_first_last', 'source_package.json']
PRODUCES = ["reports/plan_a/06_01_Scene_PlanA_SceneImageRebuild.json"]
SUGGESTED_PREVIOUS_TOOLS = ['04_02_Scene_FirstLastFrameConfirm', '05_01_Scene_ScenePromptRefresh']
SUGGESTED_NEXT_TOOLS = ['06_02_Scene_PlanA_SceneImageSelect']
...
def post_multipart_request(url: str, fields: dict[str, str], files: dict[str, Path], headers: dict[str, str], timeout: int = 120) -> dict[str, Any]:
    boundary = f"----OpenCrewBoundary{now_ms()}"
    chunks: list[bytes] = []
...
def image_b64_from_response(provider: str, payload: dict[str, Any]) -> str:
    ...
    raise ToolError("Image provider response did not include image data")
...
def generate_image(config: dict[str, str], prompt: str, output_path: Path, reference_path: Path | None = None) -> dict[str, Any]:
    ...
    if provider == "openai":
        if reference_path and reference_path.exists():
            payload = post_multipart_request("https://api.openai.com/v1/images/edits", {"model": model, "prompt": prompt, "size": "1024x1536"}, {"image": reference_path}, {"Authorization": f"Bearer {api_key}"})
        else:
        ...
        payload_data: dict[str, Any] = {"model": model, "prompt": prompt, "response_format": "b64_json"}
        inline = image_inline_payload(reference_path)
        if inline:
            payload_data["image"] = {"url": f"data:{inline['mimeType']};base64,{inline['bytesBase64Encoded']}"}
        payload = post_json_request("https://api.x.ai/v1/images/generations", payload_data, {"Authorization": f"Bearer {api_key}"})
        image_bytes = base64.b64decode(image_b64_from_response(provider, payload))
        ...
        raise ToolError(f"Unsupported image provider: {provider}/{model}")
    output_path.write_bytes(image_bytes)
    return {"provider": provider, "model": model, "output_path": str(output_path), "bytes": len(image_bytes), "reference_used": bool(reference_path and reference_path.exists())}
...
def build_image_prompt(shot: dict[str, Any], mark: dict[str, Any]) -> str:
    ...
    final_image_prompt = str(final_prompts.get("image_prompt") or "").strip()
    if final_image_prompt:
        return final_image_prompt
    text = scene_text(mark, shot_tts_source(shot).get("text") or spoken_script(shot))
    desc = mark.get("scene_description") if isinstance(mark.get("scene_description"), dict) else {}
    visual = str(desc.get("visual_prompt") or desc.get("prompt") or desc.get("summary") or "").strip()
    return (
        ...
        f"Scene visual intent: {visual}. "
        f"Shot narration/SRT intent: {strip_srt_timing(text)}"
    )
...
def scene_image_rebuild(workspace: Path, plan: dict[str, Any], source_package: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    items, blockers = [], []
    image_config = load_active_media_config(resolve_database_url(args), "image")
    for shot in target_shots(plan, args):
        shot_id = shot_id_of(shot)
        for mark in target_scene_marks(shot, args):
            scene_id = scene_id_of(mark); first_path, _ = scene_frame_paths(mark)
            output_path = canonical_scene_image_path(workspace, shot_id, scene_id); manifest_path = scene_asset_manifest_path(workspace, shot_id, scene_id)
            row = {"shot_id": shot_id, "scene_mark_id": scene_id, "first_frame": first_path, "output_image": rel(workspace, output_path), "manifest": rel(workspace, manifest_path)}
            source = resolve_existing_path(workspace, source_package, first_path)
            if not source:
                reason = "missing_first_frame" if not first_path else "first_frame_file_not_found"; blockers.append(f"{shot_id}/{scene_id}: {reason}"); items.append({**row, "status": "blocked", "blocking_reason": reason}); continue
            prompt = build_image_prompt(shot, mark)
            if not output_path.exists() or args.force:
                generation = generate_image(image_config, prompt, output_path, source)
                source_kind = "generated_image"
            else:
                generation = {"provider": image_config.get("provider", ""), "model": image_config.get("model", ""), "output_path": str(output_path), "bytes": output_path.stat().st_size}
                source_kind = "existing_generated_image"
            manifest = {**row, "status": "completed", "source": source_kind, "selected_image": rel(workspace, output_path), "reference_image": rel(workspace, source), "reference_used": generation.get("reference_used", bool(source)), "provider": generation.get("provider"), "model": generation.get("model"), "bytes": generation.get("bytes"), "prompt": prompt, "config": redact_config(image_config), "generated_at": now_ms()}
            write_json(manifest_path, manifest); mark.setdefault("plan_a", {})["scene_asset"] = {"selected_image": rel(workspace, output_path), "manifest": rel(workspace, manifest_path), "source": source_kind, "provider": generation.get("provider"), "model": generation.get("model")}; items.append(manifest)
    path = variant_shot_dir(workspace, shot_id) / "reports" / "plan_a_06_01_scene_image_rebuild.json"; payload = {"status": "completed_with_blockers" if blockers else "completed", "tool_id": TOOL_ID, "blocking_errors": blockers, "item_count": len(items), "items": items}; write_json(path, payload); return {**payload, "output": rel(workspace, path)}

def run_tool(workspace: Path, plan: dict[str, Any], source_package: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    return scene_image_rebuild(workspace, plan, source_package, args)
