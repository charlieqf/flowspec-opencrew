...
TOOL_ID = "05_01_Shot_ScenePromptRefresh"
TOOL_NAME = "Shot Scene Prompt Refresh"
TOOL_VERSION = "1.0.0"
REQUIRES = ["rebuild_shot_plan.json", "source_package.json", "confirmed_first_last", "task_id", "opencode_session_context", "run_model", "shot_id"]
...
def check_dependencies(workspace: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not missing and len(args.shot_id) == 1:
        ...
        shot = scene_tool.target_shot(plan, args.shot_id[0])
        marks = scene_tool.scene_marks_for_shot(shot or {}) if shot else []
        if not shot or not marks:
            missing.append({"dependency": "scene_marks", "reason": "target shot has no scene marks", "suggested_tools": SUGGESTED_PREVIOUS_TOOLS, "scope": scope(args)})
...
def run(workspace: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    results = [scene_tool.refresh_scene(context, source_package, shot, mark, image_workspace, int(args.timeout_seconds)) for mark in scene_tool.scene_marks_for_shot(shot)]
    scene_tool.write_json(workspace / args.output, plan)
    report = {"status": "completed_with_blockers" if any(item["status"] != "completed" for item in results) else "completed", "calibration": "image_ocr_srt_aligned", "shot_id": args.shot_id[0], "result_count": len(results), "results": results}
