...
TOOL_ID = "09_01_ShotPlan_PlanA_AssemblyBuild"
TOOL_NAME = "09_01_ShotPlan_PlanA_AssemblyBuild"
TOOL_VERSION = "1.0.0"
TOOL_LEVEL = "shotplan"
REQUIRES = ['rebuild_shot_plan.json', 'plan_a_videos']
PRODUCES = ["reports/plan_a/09_01_ShotPlan_PlanA_AssemblyBuild.json"]
SUGGESTED_PREVIOUS_TOOLS = ['08_03_ShotPlan_PlanA_ImageSequenceCompose']
...
def assembly_build(workspace: Path, plan: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    shots = []
    for shot in shot_list(plan):
        shot_id = shot_id_of(shot); video = plan_a_shot_video_path(workspace, shot_id); locked = canonical_locked_tts_path(workspace, shot_id); shots.append({"shot_id": shot_id, "shot_video": rel(workspace, video) if video.exists() else "", "locked_tts": rel(workspace, locked) if locked.exists() else ""})
    path = workspace / "reports" / "plan_a" / "assembly_plan.json"; payload = {"status": "completed", "tool_id": TOOL_ID, "output": "renders/plan_a.mp4", "shot_count": len(shots), "shots": shots}; write_json(path, payload); return {**payload, "output_path": rel(workspace, path)}

def run_tool(workspace: Path, plan: dict[str, Any], source_package: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    return assembly_build(workspace, plan, args)
