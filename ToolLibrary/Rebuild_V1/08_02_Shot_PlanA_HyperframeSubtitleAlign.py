...
TOOL_ID = "08_02_Shot_PlanA_HyperframeSubtitleAlign"
TOOL_NAME = "08_02_Shot_PlanA_HyperframeSubtitleAlign"
TOOL_VERSION = "1.0.0"
TOOL_LEVEL = "shot"
REQUIRES = ['rebuild_shot_plan.json', 'shot_tts_timeline']
PRODUCES = ["reports/plan_a/08_02_Shot_PlanA_HyperframeSubtitleAlign.json"]
SUGGESTED_PREVIOUS_TOOLS = ['08_01_Shot_PlanA_ImageSequenceClipPlan']
SUGGESTED_NEXT_TOOLS = ['08_03_Shot_PlanA_ImageSequenceCompose']
...
def srt_entry_range(entry: dict[str, Any]) -> tuple[float, float]:
    timing = str(entry.get("timing") or "")
    if "-->" not in timing:
        return 0.0, 0.0
    start, end = [part.strip() for part in timing.split("-->", 1)]
    return parse_srt_timestamp(start), parse_srt_timestamp(end)
...
def refresh_empty_timeline_image_pages(workspace: Path, shot: dict[str, Any], timeline_path: Path, timeline: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    if [item for item in timeline.get("image_pages") or [] if isinstance(item, dict)]:
        return timeline, []
    shot_id = shot_id_of(shot)
    if not scene_image_items(workspace, shot):
        return timeline, ["missing_scene_images_for_timeline_refresh"]
    locked = canonical_locked_tts_path(workspace, shot_id)
    audio_duration = ffprobe_duration(locked) if locked.exists() and locked.stat().st_size > 0 else None
    if audio_duration is None:
        audio_duration = safe_float(timeline.get("duration"), safe_float(shot.get("duration"), 1.0))
    srt_path = canonical_shot_srt_path(workspace, shot_id)
    srt_text = srt_path.read_text(encoding="utf-8") if srt_path.exists() else ensure_shot_srt_text(shot, audio_duration)
    refreshed = build_shot_tts_timeline(workspace, shot, audio_duration, srt_text)
    refreshed["refreshed_from_empty_image_pages"] = True
    refreshed["refreshed_by"] = TOOL_ID
    write_json(timeline_path, refreshed)
    return refreshed, ["refreshed_empty_timeline_image_pages"]
...
def hyperframe_subtitle_align(workspace: Path, plan: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    results, blockers = [], []
    for shot in target_shots(plan, args):
        shot_id = shot_id_of(shot); timeline_path = canonical_shot_tts_timeline_path(workspace, shot_id)
        if not timeline_path.exists(): blockers.append(f"{shot_id}: missing_shot_tts_timeline"); results.append({"shot_id": shot_id, "status": "blocked", "blocking_errors": ["missing_shot_tts_timeline"]}); continue
        timeline = read_json(timeline_path); align_path = variant_shot_dir(workspace, shot_id) / "plan_a_alignment.json"; frames = []; warnings = []
        timeline, refresh_warnings = refresh_empty_timeline_image_pages(workspace, shot, timeline_path, timeline if isinstance(timeline, dict) else {})
        warnings.extend(refresh_warnings)
        srt_entries = timeline.get("srt_entries") or []
        for page in [item for item in timeline.get("image_pages") or [] if isinstance(item, dict)]:
            image_path = resolve_workspace_path(workspace, str(page.get("image") or ""))
            if not image_path.exists() or image_path.stat().st_size <= 0:
                warnings.append(f"missing_image:{page.get('scene_mark_id')}")
            frame = {"shot_id": shot_id, "scene_mark_id": page.get("scene_mark_id"), "image": page.get("image"), "start": safe_float(page.get("start"), 0.0), "end": safe_float(page.get("end"), 0.0), "duration": safe_float(page.get("duration"), 0.0), "subtitles": []}
            for entry in srt_entries:
                if not isinstance(entry, dict) or not str(entry.get("text") or "").strip():
                    continue
                sub_start, sub_end = srt_entry_range(entry)
                if not entry.get("timing") or ranges_overlap(frame["start"], frame["end"], sub_start, sub_end):
                    frame["subtitles"].append(entry)
            frames.append(frame)
        if not frames:
            blockers.append(f"{shot_id}: empty_hyperframe_alignment"); results.append({"shot_id": shot_id, "status": "blocked", "blocking_errors": ["empty_hyperframe_alignment"]}); continue
        payload = {"shot_id": shot_id, "status": "aligned", "timeline_path": rel(workspace, timeline_path), "frames": frames, "srt_entries": srt_entries, "warnings": warnings, "generated_at": now_ms()}; write_json(align_path, payload); results.append({**payload, "alignment_path": rel(workspace, align_path)})
    path = variant_shot_dir(workspace, shot_id) / "reports" / "plan_a_08_02_hyperframe_subtitle_align.json"; payload = {"status": "completed_with_blockers" if blockers else "completed", "tool_id": TOOL_ID, "blocking_errors": blockers, "item_count": len(results), "results": results}; write_json(path, payload); return {**payload, "output": rel(workspace, path)}

def run_tool(workspace: Path, plan: dict[str, Any], source_package: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    return hyperframe_subtitle_align(workspace, plan, args)
