...
TOOL_ID = "09_02_ShotPlan_PlanA_Compose"
TOOL_NAME = "09_02_ShotPlan_PlanA_Compose"
TOOL_VERSION = "1.0.0"
TOOL_LEVEL = "shotplan"
REQUIRES = ['rebuild_shot_plan.json', 'plan_a_videos']
PRODUCES = ["reports/plan_a/09_02_ShotPlan_PlanA_Compose.json"]
SUGGESTED_PREVIOUS_TOOLS = ['09_01_ShotPlan_PlanA_AssemblyBuild']
SUGGESTED_NEXT_TOOLS = ['09_03_ShotPlan_PlanA_QualityCheck']
...
def concat_videos(videos: list[Path], output_path: Path) -> dict[str, Any]:
    if not videos:
        raise ToolError("No shot videos to concat")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    concat_path = output_path.with_suffix(".concat.txt")
    write_text(concat_path, "\n".join(concat_file_line(path) for path in videos) + "\n")
    method = "stream_copy"
    try:
        run_command([find_ffmpeg(), "-y", "-f", "concat", "-safe", "0", "-i", str(concat_path), "-c", "copy", str(output_path)])
    except ToolError:
        method = "reencode"
        run_command([find_ffmpeg(), "-y", "-f", "concat", "-safe", "0", "-i", str(concat_path), "-vf", "scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280,format=yuv420p", "-r", "24", "-c:v", "libx264", "-preset", "veryfast", "-crf", "20", "-c:a", "aac", "-b:a", "128k", str(output_path)])
    duration = ffprobe_duration(output_path)
    if not duration or duration <= 0:
        raise ToolError(f"final video duration unreadable: {output_path}")
    return {"output_path": str(output_path), "duration": duration, "shot_count": len(videos), "concat_method": method}
...
def plan_a_compose(workspace: Path, plan: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    missing, invalid, videos = [], [], []
    for shot in shot_list(plan):
        path = plan_a_shot_video_path(workspace, shot_id_of(shot))
        if not path.exists() or path.stat().st_size <= 0:
            missing.append(shot_id_of(shot))
        elif not ffprobe_duration(path):
            invalid.append(shot_id_of(shot))
        else:
            videos.append(path)
    output_path = workspace / "renders" / "plan_a.mp4"; blockers = [f"{shot}: missing_plan_a_video" for shot in missing]
    blockers.extend(f"{shot}: unreadable_plan_a_video" for shot in invalid)
    concat_result = None
    if not blockers:
        concat_result = {"output_path": str(output_path), "duration": ffprobe_duration(output_path), "source": "existing_final_render"} if output_path.exists() and output_path.stat().st_size > 0 and not args.force else concat_videos(videos, output_path)
    payload = {"status": "completed_with_blockers" if blockers else "completed", "tool_id": TOOL_ID, "blocking_errors": blockers, "output_video": rel(workspace, output_path), "exists": output_path.exists(), "video_count": len(videos), "missing_shots": missing, "invalid_shots": invalid, "concat": concat_result}; write_json(workspace / "reports" / "plan_a" / "compose.json", payload); return payload

def run_tool(workspace: Path, plan: dict[str, Any], source_package: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    return plan_a_compose(workspace, plan, args)
