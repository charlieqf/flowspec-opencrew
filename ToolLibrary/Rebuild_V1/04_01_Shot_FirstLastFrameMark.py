...
TOOL_ID = "04_01_Shot_FirstLastFrameMark"
TOOL_NAME = "Shot First Last Frame Mark"
TOOL_VERSION = "1.0.0"
REQUIRES = ["rebuild_shot_plan.json", "saved_shot_keyframes", "shot_id"]
...
SUGGESTED_NEXT_TOOLS = ["04_02_Shot_FirstLastFrameConfirm", "04_02_Scene_FirstLastFrameConfirm"]
...
def nearest_frame(frames: list[dict[str, Any]], target: float) -> dict[str, Any] | None:
    if not frames:
        return None
    return min(frames, key=lambda item: (abs(safe_float(item.get("time")) - target), str(item.get("path") or "")))
...
def ensure_scene_marks(shot: dict[str, Any], frames: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    mark = {
        ...
        "shot_id": shot_id,
        "scene_index": 1,
        "mode": "first_last" if first_path != last_path else "single",
        "generation_mode": "first_frame",
        "start": safe_float(first.get("time"), safe_float(shot.get("start"))),
        "end": safe_float(last.get("time"), safe_float(shot.get("end"))),
        "keyframes": {"single": first_path if first_path == last_path else "", "first": first_path, "last": last_path, "paths": [p for p in (first_path, last_path) if p]},
        "srt_text": str((shot.get("reference") or {}).get("srt_text") or shot.get("spoken_script") or ""),
        "warnings": ["auto_created_single_scene_from_remaining_keyframes"],
    }
...
def run(workspace: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    results = [mark_scene(shot, mark, frames, index) for index, mark in enumerate(marks, start=1)]
    sync_keyframe_scene_marks(shot)
    write_json(workspace / args.output, plan)
    return {"status": "completed_with_blockers" if any(item["status"] == "blocked" for item in results) else "completed", "shot_id": args.shot_id[0], "result_count": len(results), "results": results}


def scope(args: argparse.Namespace) -> dict[str, Any]:
    return {"shot_id": args.shot_id[0]} if args.shot_id else {}
...
def check_dependencies(workspace: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if plan_path.exists():
        ...
        if len(args.shot_id) == 1:
            try:
                shot = target_shot(read_json(plan_path), args.shot_id[0])
                if shot and keyframes_for_shot(shot):
                    satisfied.append("saved_shot_keyframes")
                else:
                    missing.append({"dependency": "saved_shot_keyframes", "reason": "target shot has no saved reference.keyframes", "suggested_tools": SUGGESTED_PREVIOUS_TOOLS, "scope": scope(args)})
