...
TOOL_ID = "08_03_Shot_PlanA_ImageSequenceCompose"
TOOL_NAME = "08_03_Shot_PlanA_ImageSequenceCompose"
TOOL_VERSION = "1.0.0"
TOOL_LEVEL = "shot"
REQUIRES = ['rebuild_shot_plan.json', 'shot_tts_timeline', 'scene_assets']
PRODUCES = ["reports/plan_a/08_03_Shot_PlanA_ImageSequenceCompose.json"]
SUGGESTED_PREVIOUS_TOOLS = ['08_02_Shot_PlanA_HyperframeSubtitleAlign']
...
def find_ffmpeg() -> str:
    for candidate in (Path("OpenCrew/.bin/ffmpeg").resolve(), Path("OpenCrew/vendor/static_ffmpeg/darwin_arm64/ffmpeg").resolve()):
        if candidate.exists():
...
def alignment_path(workspace: Path, shot_id: str) -> Path:
    return variant_shot_dir(workspace, shot_id) / "plan_a_alignment.json"

def ffmpeg_filter_path(path: Path) -> str:
    return str(path).replace("\\", "\\\\").replace(":", "\\:").replace("'", "\\'")

def subtitle_text_from_alignment(workspace: Path, shot_id: str, timeline: dict[str, Any]) -> tuple[str, str]:
    align_path = alignment_path(workspace, shot_id)
    if align_path.exists():
        alignment = read_json(align_path)
        entries: list[dict[str, Any]] = []
        seen: set[tuple[str, str]] = set()
        for frame in alignment.get("frames") or []:
            if not isinstance(frame, dict):
                continue
            for entry in frame.get("subtitles") or []:
                if isinstance(entry, dict) and str(entry.get("text") or "").strip():
                    key = (str(entry.get("timing") or ""), str(entry.get("text") or ""))
                    if key not in seen:
                        seen.add(key)
        ...
        if entries:
            ...
            for index, entry in enumerate(entries, 1):
                lines.extend([str(index), str(entry.get("timing") or ""), str(entry.get("text") or ""), ""])
            return "\n".join(lines), rel(workspace, align_path)
    shot_srt = resolve_workspace_path(workspace, str(timeline.get("shot_srt") or ""))
    if shot_srt.exists():
        return shot_srt.read_text(encoding="utf-8"), rel(workspace, shot_srt)
    return "", ""

def compose_image_sequence_video(workspace: Path, shot_id: str, timeline: dict[str, Any], audio_path: Path, output_path: Path) -> dict[str, Any]:
    pages = [item for item in timeline.get("image_pages") or [] if isinstance(item, dict)]
    if not pages:
        raise ToolError("timeline has no image_pages")
    concat_lines: list[str] = []
    input_images: list[str] = []
    for page in pages:
        image = resolve_workspace_path(workspace, str(page.get("image") or ""))
        if not image.exists() or image.stat().st_size <= 0:
            raise ToolError(f"missing image page asset: {page.get('image')}")
        duration = max(0.1, safe_float(page.get("duration"), safe_float(page.get("end"), 0.0) - safe_float(page.get("start"), 0.0)))
        concat_lines.append(concat_file_line(image))
        concat_lines.append(f"duration {duration:.3f}")
        input_images.append(rel(workspace, image))
    last_image = resolve_workspace_path(workspace, str(pages[-1].get("image") or ""))
    concat_lines.append(concat_file_line(last_image))
    concat_path = output_path.with_suffix(".images.concat.txt")
    write_text(concat_path, "\n".join(concat_lines) + "\n")
    subtitle_text, subtitle_source = subtitle_text_from_alignment(workspace, shot_id, timeline)
    subtitle_path = output_path.with_suffix(".srt")
    if subtitle_text.strip():
        write_text(subtitle_path, subtitle_text)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    video_filter = "scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280,format=yuv420p"
    if subtitle_path.exists() and subtitle_path.stat().st_size > 0:
        video_filter += f",subtitles='{ffmpeg_filter_path(subtitle_path)}'"
    run_command([
        find_ffmpeg(), "-y", "-f", "concat", "-safe", "0", "-i", str(concat_path), "-i", str(audio_path),
        "-vf", video_filter, "-r", "24",
        "-map", "0:v:0", "-map", "1:a:0", "-c:v", "libx264", "-preset", "veryfast", "-crf", "20", "-c:a", "aac", "-b:a", "128k", "-shortest", str(output_path),
    ])
    duration = ffprobe_duration(output_path)
    if not duration or duration <= 0:
        raise ToolError(f"composed video duration unreadable: {output_path}")
    return {"output_path": str(output_path), "duration": duration, "image_count": len(pages), "input_images": input_images, "audio_duration": ffprobe_duration(audio_path), "subtitle_path": rel(workspace, subtitle_path) if subtitle_path.exists() else "", "subtitle_source": subtitle_source}
...
def image_sequence_compose(workspace: Path, plan: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    results, blockers = [], []
    for shot in target_shots(plan, args):
        shot_id = shot_id_of(shot); timeline_path = canonical_shot_tts_timeline_path(workspace, shot_id); output_path = plan_a_shot_video_path(workspace, shot_id); audio_path = canonical_locked_tts_path(workspace, shot_id)
        missing = []
        if not timeline_path.exists(): missing.append("missing_shot_tts_timeline")
        if not audio_path.exists() or audio_path.stat().st_size <= 0: missing.append("missing_locked_tts")
        if missing:
            blockers.extend(f"{shot_id}: {item}" for item in missing); results.append({"shot_id": shot_id, "status": "blocked", "blocking_errors": missing, "plan_a_video": rel(workspace, output_path)}); continue
        if output_path.exists() and output_path.stat().st_size > 0 and not args.force:
            compose_result = {"output_path": str(output_path), "duration": ffprobe_duration(output_path), "source": "existing_plan_a"}
        else:
            compose_result = compose_image_sequence_video(workspace, shot_id, read_json(timeline_path), audio_path, output_path)
            compose_result["source"] = "ffmpeg_image_sequence_compose"
        results.append({"shot_id": shot_id, "status": "completed", "plan_a_video": rel(workspace, output_path), **compose_result})
    path = variant_shot_dir(workspace, shot_id) / "reports" / "plan_a_08_03_image_sequence_compose.json"; payload = {"status": "completed_with_blockers" if blockers else "completed", "tool_id": TOOL_ID, "blocking_errors": blockers, "item_count": len(results), "results": results}; write_json(path, payload); return {**payload, "output": rel(workspace, path)}

def run_tool(workspace: Path, plan: dict[str, Any], source_package: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    return image_sequence_compose(workspace, plan, args)
