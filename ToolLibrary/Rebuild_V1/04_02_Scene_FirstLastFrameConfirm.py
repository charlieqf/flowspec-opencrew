...
TOOL_ID = "04_02_Scene_FirstLastFrameConfirm"
TOOL_NAME = "Scene First Last Frame Confirm"
TOOL_VERSION = "1.0.0"
REQUIRES = ["rebuild_shot_plan.json", "marked_first_last", "shot_id", "scene_mark_id"]
...
SUGGESTED_NEXT_TOOLS = ["04_03_ShotPlan_FirstLastReadinessCheck", "05_01_Scene_ScenePromptRefresh"]
...
def check_dependencies(workspace: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not plan_path.exists():
        ...
        if len(args.shot_id) == 1 and args.scene_mark_id:
            try:
                ...
                if not shot or not mark:
                    missing.append({"dependency": "scene_mark", "reason": "target scene mark was not found", "suggested_tools": SUGGESTED_PREVIOUS_TOOLS, "scope": scope(args)})
                else:
                    ok, warnings = can_confirm(mark)
                    if ok:
                        satisfied.append("marked_first_last")
                    else:
                        missing.append({"dependency": "marked_first_last", "reason": ", ".join(warnings), "suggested_tools": SUGGESTED_PREVIOUS_TOOLS, "scope": scope(args)})
...
def run(workspace: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    result = confirm_scene(shot, mark)
    write_json(workspace / args.output, plan)
    return {"status": "completed" if result["status"] == "confirmed" else "completed_with_blockers", "results": [result]}
