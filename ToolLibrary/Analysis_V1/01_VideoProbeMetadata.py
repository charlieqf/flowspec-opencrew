...
def detect_with_opencv(video_path: Path) -> dict[str, Any]:
    if os.environ.get("OPENCREW_ANALYSIS_V1_USE_OPENCV_METADATA") != "1":
...
def merge_metadata(workspace: Path, video_path: Path, opencv_data: dict[str, Any], probe_data: dict[str, Any], ffmpeg_data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "analysis_v1_video_metadata_0.1",
...
def resolve_source_video(workspace: Path, variables: dict[str, Any]) -> Path:
    ...
    if source.is_absolute():
        raise BlockedError("source_video_path_not_relative", "Analysis_V1 tools must use a workspace-relative source_video_path.")
...
def parse_args(argv: list[str]) -> Args:
    parser = argparse.ArgumentParser(description="Probe Analysis_V1 source video metadata from SessionContext.")
