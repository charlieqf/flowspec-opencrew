# 05_02 Video Chanjing Vidu Q1 Prompt Template / ChanJing Vidu Q1视频提示词模板

适用范围：蝉镜 Vidu Q1 视频生成，重点用于首帧图片生成竖屏 9:16 口播人物视频；也支持被标记为 cutaway / product closeup 的无口播空镜段。

中文用于维护；`OPENCREW:*` 块名、字段名和变量名保持英文，块内提示词正文使用中文。

## 中文维护版

ChanJing Vidu Q1 口播任务：基于上传的首帧图片生成真实竖屏 9:16 口播人物视频。首帧是唯一视觉来源，提供人物身份、脸型、五官、发型、服装、产品或手持道具、背景、光线、镜头角度、构图、景别、透视关系和手机视频质感。

提示词只控制首帧之后的自然运动：人物如何口播、嘴型如何收敛、镜头如何稳定、表情和手势如何克制、哪些元素绝对不能变化。不要重新设计人物、服装、背景、道具或场景。

Vidu Q1 版本重点：优先追求首帧身份、构图和动作连贯性。它适合自然小幅运动和稳定口播，因此提示词必须把“第一帧一致、动作连续、不要电影化镜头、不要重构图”说清楚。不要让模型把口播段理解成剧情短片、广告镜头或大幅表演。
...
- 2026-06-21 block prefix：此模板必须使用 `VIDEO_CHANJING_VIDUQ1_*` 块名，不能复用 `VIDEO_KLING_*`。
...
- 2026-06-21 Vidu Q1 continuity：Vidu Q1 容易把首帧扩展成更电影化的运动；必须强调固定机位、连续动作、身份不漂移、背景不重构。
- 2026-06-21 expressive relaxed smile：所有口播模型统一强化松弛表情、自然笑意、略快语速和更强表现力，同时禁止大笑、露齿笑、抬头纹、皱眉和用力表演。
- 新增踩坑时只追加，不要改写历史记录；同时在 `VIDEO_CHANJING_VIDUQ1_PITFALLS_APPEND_ONLY` 中追加对应模型调用禁令。

## 模型调用版

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_POSITIVE_BASE_START -->
ChanJing Vidu Q1 图生视频任务：基于提供的首帧图片，生成真实竖屏 9:16 视频。
...
Vidu Q1 运动策略：动作从首帧自然延续，只做低幅度、连续、真实的人体微动作。保持手机口播质感，不要生成电影感运镜、广告大片质感、剧情表演、舞台表演或夸张情绪变化。

目标时长：约 {{duration_seconds}} 秒。
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_POSITIVE_BASE_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_DIALOGUE_STANDARD_START -->
...
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_DIALOGUE_STANDARD_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_DIALOGUE_CUTAWAY_START -->
...
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_DIALOGUE_CUTAWAY_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_CAMERA_LOCK_START -->
镜头锁定，固定机位，保持首帧原始景别、视线高度、主体位置、主体比例、背景、画面边缘和构图不变。

全程不要推镜、拉镜、摇镜、移镜、平移、旋转、变焦、自动重构图、画面漂移、视角变化、跳切、切镜、手持漂移或场景变化。
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_CAMERA_LOCK_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_PERFORMANCE_START -->
...
手部动作必须与口播节奏自然匹配，动作之间有停顿，不要连续乱摆、突然抬高手臂、双手交叉穿模、手指闪烁或从画面边缘生成新的手。
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_PERFORMANCE_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_STATIC_OBJECTS_START -->
...
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_STATIC_OBJECTS_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_NEGATIVE_BASE_START -->
...
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_NEGATIVE_BASE_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_NEGATIVE_CAMERA_START -->
镜头运动、手持晃动、轻微晃动、画面漂移、自动重构图、推镜、拉镜、摇镜、移镜、平移、旋转、变焦、视角变化、构图变化、跳切、切镜、场景变化、快速运动
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_NEGATIVE_CAMERA_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_NEGATIVE_EXPRESSION_START -->
...
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_NEGATIVE_EXPRESSION_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_NEGATIVE_OBJECTS_START -->
产品漂移、产品变形、产品融化、产品复制、产品消失、产品旋转、标签变化、文字变形、包装融化、包装改字、屏幕内容变化、Logo 变形、平面人脸眨眼、平面人脸张嘴、平面人脸说话
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_NEGATIVE_OBJECTS_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_PITFALLS_APPEND_ONLY_START -->
...
电影化运镜、广告大片质感、剧情化表演、舞台表演、重构图、背景重建、身份漂移

动作断裂、动作不连续、突然大幅移动、身体突然前倾、突然靠近镜头、连续乱摆手

表情僵硬、表情紧绷、表情过度、明显假笑、咧嘴笑、露齿大笑、酒窝、苹果肌过重、抬头纹

语速拖沓、语速过慢、嘴型夸张、嘴巴张太大、下巴变形、口型漂移、异常牙齿、手部变形
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_PITFALLS_APPEND_ONLY_END -->

<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_PROMPT_START -->
{{positive_prompt}}

Negative prompt:
{{negative_prompt}}
<!-- OPENCREW:VIDEO_CHANJING_VIDUQ1_PROMPT_END -->
...
Vidu Q1 需要保持连续真实的手机口播质感：不要把画面变成电影片段、广告片、剧情表演或大幅镜头运动；人物动作从首帧自然延续，手势低幅度、有停顿、不乱摆。
