# 05_02 Video Chanjing Doubao Prompt Template / ChanJing Doubao视频提示词模板

适用范围：蝉镜 Doubao 视频生成，重点用于首帧图片生成竖屏 9:16 口播人物视频；也支持被标记为 cutaway / product closeup 的无口播空镜段。

中文用于维护；`OPENCREW:*` 块名、字段名和变量名保持英文，块内提示词正文使用中文。

## 中文维护版

ChanJing Doubao 口播任务：基于上传的首帧图片生成真实竖屏 9:16 口播人物视频。首帧是唯一视觉来源，提供人物身份、脸型、五官、发型、服装、产品或手持道具、背景、光线、镜头角度、构图、景别、透视关系和手机视频质感。

提示词只控制首帧之后的自然运动：人物如何口播、嘴型如何收敛、镜头如何稳定、表情和手势如何克制、哪些元素绝对不能变化。不要重新设计人物、服装、背景、道具或场景。

Doubao / Seedance 版本重点：允许根据台词语义生成更自然的节奏和强调动作，但必须把动作约束在口播人物的真实范围内。不要让模型因为语义联想添加新场景、新物体、屏幕文字、情绪化剧情或强烈镜头运动。
...
- 2026-06-21 block prefix：此模板必须使用 `VIDEO_CHANJING_DOUBAO_*` 块名，不能复用 `VIDEO_KLING_*`。
...
- 2026-06-21 Doubao semantic control：Doubao / Seedance 可能根据台词扩展场景或生成文字；必须强调台词只驱动口播节奏，不驱动新增画面元素。
- 2026-06-21 expressive relaxed smile：所有口播模型统一强化松弛表情、自然笑意、略快语速和更强表现力，同时禁止大笑、露齿笑、抬头纹、皱眉和用力表演。
- 新增踩坑时只追加，不要改写历史记录；同时在 `VIDEO_CHANJING_DOUBAO_PITFALLS_APPEND_ONLY` 中追加对应模型调用禁令。

## 模型调用版

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_POSITIVE_BASE_START -->
ChanJing Doubao 图生视频任务：基于提供的首帧图片，生成真实竖屏 9:16 视频。
...
Doubao / Seedance 运动策略：台词可以作为表情和手势节奏参考，但只能产生真实口播中的松弛微表情、自然笑意、轻微点头和胸口级强调动作。表现力要比普通口播更强，但不要把台词内容画成画面，不要生成隐喻画面、剧情画面、病理图、文字海报或信息图。

目标时长：约 {{duration_seconds}} 秒。
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_POSITIVE_BASE_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_DIALOGUE_STANDARD_START -->
...
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_DIALOGUE_STANDARD_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_DIALOGUE_CUTAWAY_START -->
...
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_DIALOGUE_CUTAWAY_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_CAMERA_LOCK_START -->
镜头锁定，固定机位，保持首帧原始景别、视线高度、主体位置、主体比例、背景、画面边缘和构图不变。

全程不要推镜、拉镜、摇镜、移镜、平移、旋转、变焦、自动重构图、画面漂移、视角变化、跳切、切镜、手持漂移或场景变化。
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_CAMERA_LOCK_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_PERFORMANCE_START -->
...
手势应跟随台词重音自然出现，例如轻微抬手、手掌小幅打开、短暂停顿后收回。不要指向镜头、不要敲桌、不要连续比划复杂形状、不要让手势解释成新增道具或文字。
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_PERFORMANCE_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_STATIC_OBJECTS_START -->
...
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_STATIC_OBJECTS_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_NEGATIVE_BASE_START -->
...
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_NEGATIVE_BASE_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_NEGATIVE_CAMERA_START -->
镜头运动、手持晃动、轻微晃动、画面漂移、自动重构图、推镜、拉镜、摇镜、移镜、平移、旋转、变焦、视角变化、构图变化、跳切、切镜、场景变化、快速运动
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_NEGATIVE_CAMERA_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_NEGATIVE_EXPRESSION_START -->
...
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_NEGATIVE_EXPRESSION_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_NEGATIVE_OBJECTS_START -->
产品漂移、产品变形、产品融化、产品复制、产品消失、产品旋转、标签变化、文字变形、包装融化、包装改字、屏幕内容变化、Logo 变形、平面人脸眨眼、平面人脸张嘴、平面人脸说话
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_NEGATIVE_OBJECTS_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_PITFALLS_APPEND_ONLY_START -->
- 2026-06-21 block prefix：模板块名必须是 `VIDEO_CHANJING_DOUBAO_*`。
...
- 2026-06-21 Doubao semantic control：台词只驱动口播节奏和轻微手势，不要生成语义插画、字幕、屏幕文字、新场景或新道具。
- 2026-06-21 expressive relaxed smile：表情松弛，面带自然笑意，语速略快于一般人，表现力强但不过度用力。
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_PITFALLS_APPEND_ONLY_END -->

<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_PROMPT_START -->
{{positive_prompt}}

Negative prompt:
{{negative_prompt}}
<!-- OPENCREW:VIDEO_CHANJING_DOUBAO_PROMPT_END -->
...
Doubao / Seedance 只用台词指导口播节奏和轻微强调动作，不要根据台词生成新的物体、文字、信息图、病理图、剧情画面或明显情绪化表演。
