...
TEMPLATE_NAME = "Ref_05_02_Video_ChanJing_Kling.md"
SOURCE_TEMPLATE_PATH = Path(__file__).resolve().parents[1] / "Reference" / "05_02" / "Video_ChanJing_Kling.md"
API_BASE_URL = "https://open-api.chanjing.cc/open/v1"
DEFAULT_MODEL = "kling2.5"
CHANJING_KLING_MODELS = {"kling2.5", "kling-v2-1-master", "kling1.6"}
...
def _block(template_text: str, name: str) -> str:
    ...
    if start not in template_text or end not in template_text:
        raise ToolError(f"Video Chanjing Kling template is missing block marker: {name}")
...
def build_prompt_package(context: dict[str, Any]) -> dict[str, Any]:
    ...
    positive_blocks = [
        "VIDEO_CHANJING_KLING_POSITIVE_BASE",
        "VIDEO_CHANJING_KLING_DIALOGUE_CUTAWAY" if cutaway else "VIDEO_CHANJING_KLING_DIALOGUE_STANDARD",
        "VIDEO_CHANJING_KLING_CAMERA_LOCK",
        "VIDEO_CHANJING_KLING_STATIC_OBJECTS" if cutaway else "VIDEO_CHANJING_KLING_PERFORMANCE",
        "VIDEO_CHANJING_KLING_STATIC_OBJECTS" if not cutaway else "",
    ]
    negative_blocks = [
        "VIDEO_CHANJING_KLING_NEGATIVE_BASE",
        "VIDEO_CHANJING_KLING_NEGATIVE_CAMERA",
        "VIDEO_CHANJING_KLING_NEGATIVE_EXPRESSION" if not cutaway else "",
        "VIDEO_CHANJING_KLING_NEGATIVE_OBJECTS",
        "VIDEO_CHANJING_KLING_PITFALLS_APPEND_ONLY",
    ]
    positive = _join(template_text, positive_blocks, variables)
    negative = _join(template_text, negative_blocks, variables)
    prompt = _render(_block(template_text, "VIDEO_CHANJING_KLING_PROMPT"), {**variables, "positive_prompt": positive, "negative_prompt": negative})
    return {
        "schema_version": "analysis_v1_05_02_video_prompt_chanjing_kling_0.1",
        "prompt_type": "video_generation",
        "provider_profile": "video_chanjing_kling",
        ...
        "template_blocks": [name for name in positive_blocks + negative_blocks + ["VIDEO_CHANJING_KLING_PROMPT"] if text_value(name)],
...
def normalize_model(model: str) -> str:
    ...
    aliases = {
        ...
        "kling2.1-master": "kling-v2-1-master",
        "kling-1.6": "kling1.6",
    }
    normalized = aliases.get(value.lower(), value)
    if normalized not in CHANJING_KLING_MODELS:
        raise ToolError(f"Unsupported Chanjing Kling model: {model}. Expected one of {sorted(CHANJING_KLING_MODELS)}.")
    return normalized
...
def matching_task_state(path: Path | None, fingerprint: str, model: str) -> dict[str, Any]:
    ...
    if text_value(state.get("provider")) not in {"", "chanjing"}:
        return {}
    if text_value(state.get("provider_profile")) not in {"", "video_chanjing_kling"}:
        return {}
    return state
...
def download_video(requests: Any, video_url: str, output_path: Path) -> int:
    ...
    if safe_download_to_path is not None:
        safe_download_to_path(
            ...
            timeout=600,
            headers={"User-Agent": "OpenCrew/chanjing-kling-video-download"},
        )
...
def generate(context: dict[str, Any], prompt_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    if provider not in {"chanjing", "chanjing.cc", "cj"}:
        raise ToolError(f"Unsupported Chanjing Kling provider: {provider}/{config.get('model')}")
    ...
    if not reference_images:
        raise ToolError("Chanjing Kling video generation requires one first-frame image.")
    ...
    if prior_unique_id:
        ...
        write_task_state(state_path, {
            "schema_version": "analysis_v1_chanjing_kling_provider_task_0.1",
            "provider": "chanjing",
            "provider_profile": "video_chanjing_kling",
            "model": model,
