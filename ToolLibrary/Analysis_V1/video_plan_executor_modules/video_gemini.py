...
TEMPLATE_NAME = "Ref_05_02_Video_Gemini.md"
SOURCE_TEMPLATE_PATH = Path(__file__).resolve().parents[1] / "Reference" / "05_02" / "Video_Gemini.md"
...
def _block(template_text: str, name: str) -> str:
    ...
    if start not in template_text or end not in template_text:
        raise ToolError(f"Video Gemini template is missing block marker: {name}")
...
def build_prompt_package(context: dict[str, Any]) -> dict[str, Any]:
    ...
    positive_blocks = ["VIDEO_GEMINI_POSITIVE_BASE", "VIDEO_GEMINI_DIALOGUE_CUTAWAY" if cutaway else "VIDEO_GEMINI_DIALOGUE_STANDARD"]
    negative_blocks = ["VIDEO_GEMINI_NEGATIVE_BASE", "VIDEO_GEMINI_NEGATIVE_CUTAWAY" if cutaway else "", "VIDEO_GEMINI_PITFALLS_APPEND_ONLY"]
    positive = _join(template_text, positive_blocks, variables)
    negative = _join(template_text, negative_blocks, variables)
    prompt = _render(_block(template_text, "VIDEO_GEMINI_PROMPT"), {**variables, "positive_prompt": positive, "negative_prompt": negative})
    return {
        "schema_version": "analysis_v1_05_02_video_prompt_gemini_0.1",
        "prompt_type": "video_generation",
        "provider_profile": "video_gemini",
        ...
        "template_blocks": [name for name in positive_blocks + negative_blocks + ["VIDEO_GEMINI_PROMPT"] if text_value(name)],
...
def generate(context: dict[str, Any], prompt_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    if not api_key:
        raise ToolError(f"Missing video API key for gemini/{model}.")
    ...
    operation = post_json_request(endpoint, {"instances": [instance], "parameters": {"sampleCount": 1, "durationSeconds": seconds, "aspectRatio": aspect}}, {}, timeout=120)
    op_name = text_value(operation.get("name"))
    if not op_name:
        raise ToolError(f"Gemini video response did not include operation name: {json_safe(operation)}")
    ...
    while time.time() < deadline:
        ...
        time.sleep(5)
    if not video_url:
        raise ProviderTimeout("Gemini video generation timed out or completed without URL.")
    download_binary(video_url, output_path, {"x-goog-api-key": api_key})
    return {"provider": "gemini", "model": model, "requested_duration": duration, "duration": seconds, "aspect": aspect, "output_path": str(output_path), "video_url": video_url, "elapsed_seconds": round(time.time() - started_at, 3)}
