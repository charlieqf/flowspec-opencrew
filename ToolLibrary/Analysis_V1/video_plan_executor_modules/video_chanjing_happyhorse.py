...
TEMPLATE_NAME = "Ref_05_02_Video_ChanJing_HappyHorse.md"
SOURCE_TEMPLATE_PATH = Path(__file__).resolve().parents[1] / "Reference" / "05_02" / "Video_ChanJing_HappyHorse.md"
API_BASE_URL = "https://open-api.chanjing.cc/open/v1"
DEFAULT_MODEL = "happyhorse-1.0-i2v"
CHANJING_HAPPYHORSE_MODELS = {"happyhorse-1.0-t2v", "happyhorse-1.0-i2v", "happyhorse-1.0-r2v", "happyhorse-1.0-video-edit"}
...
def _block(template_text: str, name: str) -> str:
    ...
    if start not in template_text or end not in template_text:
        raise ToolError(f"Video Chanjing HappyHorse template is missing block marker: {name}")
...
def build_prompt_package(context: dict[str, Any]) -> dict[str, Any]:
    ...
    positive_blocks = [
        "VIDEO_CHANJING_HAPPYHORSE_POSITIVE_BASE",
        "VIDEO_CHANJING_HAPPYHORSE_DIALOGUE_CUTAWAY" if cutaway else "VIDEO_CHANJING_HAPPYHORSE_DIALOGUE_STANDARD",
        "VIDEO_CHANJING_HAPPYHORSE_CAMERA_LOCK",
        "VIDEO_CHANJING_HAPPYHORSE_STATIC_OBJECTS" if cutaway else "VIDEO_CHANJING_HAPPYHORSE_PERFORMANCE",
        "VIDEO_CHANJING_HAPPYHORSE_STATIC_OBJECTS" if not cutaway else "",
    ]
    negative_blocks = [
        "VIDEO_CHANJING_HAPPYHORSE_NEGATIVE_BASE",
        "VIDEO_CHANJING_HAPPYHORSE_NEGATIVE_CAMERA",
        "VIDEO_CHANJING_HAPPYHORSE_NEGATIVE_EXPRESSION" if not cutaway else "",
        "VIDEO_CHANJING_HAPPYHORSE_NEGATIVE_OBJECTS",
        "VIDEO_CHANJING_HAPPYHORSE_PITFALLS_APPEND_ONLY",
    ]
    positive = _join(template_text, positive_blocks, variables)
    negative = _join(template_text, negative_blocks, variables)
    prompt = _render(_block(template_text, "VIDEO_CHANJING_HAPPYHORSE_PROMPT"), {**variables, "positive_prompt": positive, "negative_prompt": negative})
    return {
        "schema_version": "analysis_v1_05_02_video_prompt_chanjing_happyhorse_0.1",
        "prompt_type": "video_generation",
        "provider_profile": "video_chanjing_happyhorse",
        ...
        "template_blocks": [name for name in positive_blocks + negative_blocks + ["VIDEO_CHANJING_HAPPYHORSE_PROMPT"] if text_value(name)],
...
def normalize_model(model: str) -> str:
    ...
    if normalized not in CHANJING_HAPPYHORSE_MODELS:
        raise ToolError(f"Unsupported Chanjing HappyHorse model: {model}. Expected one of {sorted(CHANJING_HAPPYHORSE_MODELS)}.")
    return normalized
...
def reference_image_limit(model: str) -> int:
    return 3 if text_value(model).lower() == "happyhorse-1.0-r2v" else 1


def request_payload(prompt: str, model: str, image_asset: dict[str, Any], seconds: int, config: dict[str, Any], reference_image_assets: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    elif text_value(model).lower() == "happyhorse-1.0-r2v":
        refs = [
            text_value(asset.get("full_path"))
            for asset in reference_image_assets or []
            if text_value(asset.get("full_path"))
        ]
        if refs:
            payload["ref_img_url"] = refs[:2]
    return payload


def request_fingerprint(prompt: str, model: str, reference_images: list[Path], seconds: int, payload: dict[str, Any]) -> str:
    image_limit = reference_image_limit(model)
    public_payload = {
        "provider": "chanjing",
        "profile": "video_chanjing_happyhorse",
        "model": model,
        "prompt_sha256": hashlib.sha256(prompt.encode("utf-8")).hexdigest(),
        "reference_image_sha256": [file_sha256(path) for path in reference_images[:image_limit]],
        "ref_img_url_count": len(payload.get("ref_img_url") or []),
...
def matching_task_state(path: Path | None, fingerprint: str, model: str) -> dict[str, Any]:
    ...
    if text_value(state.get("provider")) not in {"", "chanjing"}:
        return {}
    if text_value(state.get("provider_profile")) not in {"", "video_chanjing_happyhorse"}:
        return {}
    return state
...
def download_video(requests: Any, video_url: str, output_path: Path) -> int:
    ...
    if safe_download_to_path is not None:
        safe_download_to_path(
            ...
            timeout=600,
            headers={"User-Agent": "OpenCrew/chanjing-happyhorse-video-download"},
        )
...
def generate(context: dict[str, Any], prompt_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    if provider not in {"chanjing", "chanjing.cc", "cj"}:
        raise ToolError(f"Unsupported Chanjing HappyHorse provider: {provider}/{config.get('model')}")
    ...
    if not reference_images:
        raise ToolError("Chanjing HappyHorse video generation requires at least one reference image.")
    image_limit = reference_image_limit(model)
    selected_reference_images = reference_images[:image_limit]
    ...
    image_assets = [upload_asset(requests, config, access_token, path, "ai_creation", timeout=180) for path in selected_reference_images]
    image_asset = image_assets[0]
    payload = request_payload(prompt, model, image_asset, seconds, config, image_assets[1:])
    fingerprint = request_fingerprint(prompt, model, selected_reference_images, seconds, payload)
    ...
    if prior_unique_id and text_value(prior_state.get("status")).lower() in {"success", "succeeded", "completed"} and output_path.exists() and output_path.stat().st_size > 0:
        return {
            ...
            "elapsed_seconds": 0,
            "image_assets": list_value(prior_state.get("image_assets")),
            "cached": True,
        }
    ...
    if prior_unique_id:
        ...
        write_task_state(state_path, {
            "schema_version": "analysis_v1_chanjing_happyhorse_provider_task_0.1",
            "provider": "chanjing",
            "provider_profile": "video_chanjing_happyhorse",
            "model": model,
