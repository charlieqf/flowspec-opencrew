...
try:
    from OpenCrew.ToolLibrary.Analysis.media_binaries import find_ffmpeg, media_env
except Exception:  # pragma: no cover - standalone fallback when package import path is unusual
    ANALYSIS_DIR = Path(__file__).resolve().parents[1] / "Analysis"
    if str(ANALYSIS_DIR) not in sys.path:
        sys.path.insert(0, str(ANALYSIS_DIR))
    from media_binaries import find_ffmpeg, media_env  # type: ignore
...
def analysis_v1_runtime_python_candidates() -> list[Path]:
    candidates: list[Path | None] = [
        ...
        opencrew_data_dir() / "runtimes" / "analysis_v1_py312" / "bin" / "python",
...
def maybe_reexec_with_local_whisper_runtime() -> None:
    env_flag = "ANALYSIS_V1_LOCAL_WHISPER_RUNTIME_REEXEC"
    if os.environ.get(env_flag) == "1":
        return
    python = next((candidate for candidate in analysis_v1_runtime_python_candidates() if python_can_import(candidate, "whisper")), None)
    if python is None:
        return
...
def build_quality(segments: list[dict[str, Any]], duration: float, provider_payload: dict[str, Any], warnings: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not segments or not text.strip():
        ...
        quality_level = "good"
    return {
        "schema_version": "analysis_v1_asr_quality_0.1",
        "status": "checked",
...
def base_result(workspace: Path, args: Args) -> dict[str, Any]:
    return {
        ...
        "runtime": {
            ...
            "runtime_reexec": os.environ.get("ANALYSIS_V1_LOCAL_WHISPER_RUNTIME_REEXEC") == "1",
        },
...
def run(args: Args) -> dict[str, Any]:
    ...
    try:
        ...
        if reusable is not None:
            ...
            segments_payload = {
                "schema_version": "analysis_v1_asr_segments_0.1",
        ...
        add_block(
            ...
            f"Cannot read/write Analysis_V1 workspace. 请授权 file_system read/write: {OPENCREW_HOME} 后重试。原始错误: {exc}",
        )
...
def parse_args(argv: list[str]) -> Args:
    parser = argparse.ArgumentParser(description="Run Analysis_V1 audio ASR and emit video-timeline-aligned SRT.")
