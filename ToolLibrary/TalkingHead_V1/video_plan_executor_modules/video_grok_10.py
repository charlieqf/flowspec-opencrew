...
TEMPLATE_NAME = "Ref_05_02_Video_Grok_10.md"
SOURCE_TEMPLATE_PATH = Path(__file__).resolve().parents[1] / "Reference" / "05_02" / "Video_Grok_10.md"
...
def normalize_video_aspect(value: Any) -> str:
    aspect = text_value(value)
    if aspect in {"9:16", "16:9"}:
        return aspect
    raise ToolError("Video aspect ratio must come from the reference image or be explicitly set to 9:16 or 16:9.")


def video_aspect_for_context(context: dict[str, Any]) -> str:
    ...
    if image_path and image_path.is_file() and Image is not None:
        try:
            with Image.open(image_path) as source:
                image = ImageOps.exif_transpose(source) if ImageOps is not None else source
            ...
            pass
    return normalize_video_aspect(context.get("aspect") or context.get("aspect_ratio") or context.get("requested_aspect"))
...
def build_prompt_package(context: dict[str, Any]) -> dict[str, Any]:
    ...
    aspect = video_aspect_for_context(context)
    variables = {
    ...
    context_block = "VIDEO_GROK_CONTEXT_CUTAWAY" if cutaway else "VIDEO_GROK_CONTEXT_TALKING_HEAD"
    context_blocks = [context_block] if _has_block(template_text, context_block) else []
    ...
    return {
        "schema_version": "talking_head_v1_05_02_video_prompt_grok_10_0.1",
        "prompt_type": "video_generation",
        "provider_profile": "video_grok_10",
        ...
        "template_blocks": [name for name in [
            "Video_Grok_10.FIRST_FRAME_ONLY",
            *positive_blocks,
            *negative_blocks,
            "VIDEO_GROK_PROMPT",
        ] if text_value(name)],
        ...
        "negative_prompt": negative,
        "prompt": prompt,
        "extracted_fields": {"shot_id": text_value(shot.get("shot_id")), "scene_id": text_value(scene.get("scene_id")), "dialogue_text": text, "duration": duration, "aspect_ratio": aspect, "cutaway": cutaway},
    }
