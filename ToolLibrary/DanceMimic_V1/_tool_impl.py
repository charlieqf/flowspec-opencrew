...
PRIVACY_GRID_MAX_CELL_SIZE_REFERENCE = 12
    ...
    return {
        ...
        "reference_face_masked_video_build": {
            ...
            "privacy_grid": {
                ...
                "cell_size_reference": PRIVACY_GRID_MAX_CELL_SIZE_REFERENCE,
        ...
        try:
            ...
            if engine == "insightface_scrfd":
                ...
                for name, cascade, confidence in opencv_haar_cascades(cv2):
                    for x, y, w, h in cascade.detectMultiScale(gray, scaleFactor=1.08, minNeighbors=3, minSize=(20, 20)):
                        candidates.append({"bbox": [x, y, w, h], "confidence": confidence, "engine": engine, "cascade": name})
...
def privacy_grid_visual(config: dict[str, Any], width: int, height: int) -> tuple[int, int, tuple[int, int, int]]:
    grid = dict_value(config.get("privacy_grid"))
    scale = max(0.5, min(width, height) / 1080.0)
    line_width = max(1, min(3, int(round(float_value(grid.get("line_width_reference"), 1.0) * scale))))
    configured_cell = float_value(grid.get("cell_size_reference"), PRIVACY_GRID_MAX_CELL_SIZE_REFERENCE)
    cell_reference = min(configured_cell, PRIVACY_GRID_MAX_CELL_SIZE_REFERENCE)
    cell = max(12, int(round(cell_reference * scale)))
...
def privacy_grid_line_presence(image: Any, bbox: list[int], config: dict[str, Any], np: Any) -> float:
    ...
    if pixels.size == 0:
        return 0.0
    red = (pixels[:, 2] >= 180) & (pixels[:, 2] >= pixels[:, 1] * 1.5) & (pixels[:, 2] >= pixels[:, 0] * 1.5)
...
def run_02(workspace: Path, args: Args) -> dict[str, Any]:
    ...
    config["privacy_grid"]["cell_size_reference"] = min(
        float_value(config["privacy_grid"].get("cell_size_reference"), PRIVACY_GRID_MAX_CELL_SIZE_REFERENCE),
    ...
    if mode == "red_grid_guide":
        ...
        privacy_manifest = {
            ...
            "render": {"line_color": "#ff1f1f", "line_width_reference": int_value(grid_config.get("line_width_reference"), 1), "cell_size_reference": int_value(grid_config.get("cell_size_reference"), PRIVACY_GRID_MAX_CELL_SIZE_REFERENCE), "fill_alpha": 0},
            "created_at": now_iso(),
        }
