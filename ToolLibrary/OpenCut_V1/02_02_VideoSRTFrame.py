...
def opencut_v1_runtime_python_candidates() -> list[Path]:
    candidates: list[Path | None] = [
        ...
        Path(os.environ.get("OPENCREW_DATA_DIR") or (Path.home() / ".opencrew")).expanduser() / "runtimes" / "opencut_v1_py312" / "bin" / "python",
...
def maybe_reexec_with_rapidocr_runtime(requested: str) -> None:
    env_flag = "OPENCUT_V1_RAPIDOCR_RUNTIME_REEXEC"
    if os.environ.get(env_flag) == "1" or current_runtime_has_ocr(requested):
        return
    if requested not in {"auto", "rapidocr"}:
        return
    python = next((candidate for candidate in opencut_v1_runtime_python_candidates() if python_can_import_rapidocr(candidate)), None)
    if python is None:
        return
...
def build_final_srt_frame_items(calibrated: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "opencut_v1_final_srt_frame_items_0.2",
        "items": items,
    }
...
def base_result(workspace: Path, args: Args) -> dict[str, Any]:
    return {
        ...
        "runtime": {
            ...
            "runtime_reexec": os.environ.get("OPENCUT_V1_RAPIDOCR_RUNTIME_REEXEC") == "1",
        },
...
def run(args: Args) -> dict[str, Any]:
    ...
    try:
        ...
        if reusable is not None:
            ...
            frame_map = {
                "schema_version": "opencut_v1_srt_frame_map_0.1",
                **common,
                "items": items,
            }
            sentence_index = {
                "schema_version": "opencut_v1_srt_sentence_index_0.1",
                **common,
                "items": [
                    {
            ...
            calibrated = {
                "schema_version": "opencut_v1_calibrated_srt_items_0.1",
                **common,
        ...
        add_block(result, "workspace_permission_denied", f"Cannot read/write OpenCut_V1 workspace. Original error: {exc}")
    except Exception as exc:
