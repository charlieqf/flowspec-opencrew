...
OPENCUT_V1_PYTHON_ENV = "OPENCREW_OPENCUT_V1_PYTHON"
OPENCUT_V1_RUNTIME_DIR_ENV = "OPENCREW_OPENCUT_V1_RUNTIME_DIR"
OPENCREW_DATA_DIR_ENV = "OPENCREW_DATA_DIR"
LEGACY_CONTEXT_LINK_SUFFIXES = {
    ".aac",
    ".avi",
    ".flac",
    ".m4a",
...
try:
    ...
    raise RuntimeError("OpenCut_V1 framework bridge requires opcrew_backend on PYTHONPATH") from exc
...
def _prepare_legacy_context(root: Path) -> None:
    ...
    for source in context_dir.iterdir():
        ...
        if target.exists():
            continue
        if source.is_file():
            if source.suffix.lower() in LEGACY_CONTEXT_LINK_SUFFIXES:
                # prepare.py already owns the run-isolated snapshot in
                # 0_SessionContext. The legacy context is only a compatibility
                # alias inside this tool-use session, so avoid a second
                # physical media copy when hard links are available.
                try:
    ...
    if not missing:
        missing.append(
            MissingDependency(
                ...
                suggested_action="Legacy OpenCut_V1 tool returned blocked without structured blocked_reasons.",
            )
        )
...
def _opencut_v1_runtime_python_candidates(env: dict[str, str]) -> list[Path]:
    candidates: list[Path | None] = [
        Path(env[OPENCUT_V1_PYTHON_ENV]).expanduser() if env.get(OPENCUT_V1_PYTHON_ENV) else None,
        Path(env[OPENCUT_V1_RUNTIME_DIR_ENV]).expanduser() / "bin" / "python" if env.get(OPENCUT_V1_RUNTIME_DIR_ENV) else None,
        Path(env.get(OPENCREW_DATA_DIR_ENV) or (Path.home() / ".opencrew")).expanduser() / "runtimes" / "opencut_v1_py312" / "bin" / "python",
...
def _select_legacy_python(env: dict[str, str]) -> str:
    python = next((candidate for candidate in _opencut_v1_runtime_python_candidates(env) if candidate.exists()), None)
    if python is None:
        return sys.executable
    if not env.get(OPENCUT_V1_PYTHON_ENV):
        env[OPENCUT_V1_PYTHON_ENV] = str(python)
    if not env.get(OPENCUT_V1_RUNTIME_DIR_ENV) and python.name == "python" and python.parent.name == "bin":
        env[OPENCUT_V1_RUNTIME_DIR_ENV] = str(python.parent.parent)
    return str(python)
