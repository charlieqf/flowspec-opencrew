...
from opcrew_backend.koubo.koubo_storyboard.asset_core_services import register_asset_core_services  # noqa: E402
...
def ensure_working_assets(item: dict[str, Any], **_sc_kwargs) -> dict[str, Any]:
...
def ensure_dialogue_working_assets(item: dict[str, Any], **_sc_kwargs) -> dict[str, Any]:
    return ensure_working_assets(item)


def dialogue_asset_key(item: dict[str, Any], **_sc_kwargs) -> str:
    return text(item.get("srt_id") or item.get("dialogue_id") or "dialogue")


def derive_dialogue_asset_key(item: dict[str, Any], used_keys: set[str] | None = None) -> str:
    used = used_keys if isinstance(used_keys, set) else set()
    explicit = text(item.get("dialogue_asset_key"))
    if explicit and explicit not in used:
        return explicit
    candidate = dialogue_asset_key(item)
    if candidate and candidate not in used:
        return candidate
    index = len(used) + 1
    while True:
        candidate = f"dlg_{index:04d}"
...
class KouboStoryboardStaleEditContractTest(unittest.TestCase):
    def make_services(self, workspace: Path) -> SimpleNamespace:
        ns = SimpleNamespace(
            ...
            derive_dialogue_asset_key=derive_dialogue_asset_key,
            asset_pool_meta=lambda _workspace, **_sc_kwargs: {
                "uploaded_images": [],
                "uploaded_audios": [],
                "uploaded_videos": [],
                "history_versions": [],
            ...
            source_asset_groups_from_source=lambda _workspace, _source, **_sc_kwargs: [],
        )
    ...
    def test_load_plan_ignores_legacy_edit_from_older_storyboard_tool_output(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plan, meta = services.load_plan({"id": 112, "session_id": 171}, sc=services)
    ...
    def test_projected_source_keeps_saved_edit_current_across_repeated_saves(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            source = source_storyboard("04_02_StoryBoard", "原始文案")
            source_hash = storyboard_plan_services.stable_storyboard_sha256(source)
            write_json(workspace / "SessionOutput/storyboard/srt_storyboard.json", source)
            services = self.make_services(workspace)
            plan = edit_storyboard("04_02_StoryBoard", "第一次保存后的文案", source_hash)

            services.save_edit_and_source_storyboard(
                {"id": 112, "session_id": 171}, workspace, plan, sc=services,
            )

            projected = read_json(workspace / "SessionOutput/storyboard/srt_storyboard.json")
            self.assertEqual(projected["updated_from"], "SessionOutput/storyboard/koubo_storyboard_edit.json")
            self.assertEqual(projected["source_storyboard_sha256"], source_hash)
            self.assertNotEqual(storyboard_plan_services.stable_storyboard_sha256(projected), source_hash)

            loaded, meta = services.load_plan({"id": 112, "session_id": 171}, sc=services)
            self.assertEqual(loaded["shots"][0]["scenes"][0]["dialogues"][0]["text"], "第一次保存后的文案")
            self.assertEqual(loaded["source_storyboard_sha256"], source_hash)
            self.assertEqual(meta["has_saved_edit"], True)
            self.assertEqual(meta["stale_edit_ignored"], False)
            ...
            loaded["storyboard_tts_selection"] = {
                **trusted_candidate,
                "tempo": 0.7,
                "source": "storyboard_audio_settings",
                "top_candidates": [trusted_candidate],
                "recommendations": [trusted_candidate],
            }
            services.save_edit_and_source_storyboard(
                {"id": 112, "session_id": 171}, workspace, loaded, sc=services,
            )
            reloaded, reloaded_meta = services.load_plan({"id": 112, "session_id": 171}, sc=services)
            self.assertEqual(reloaded["storyboard_tts_selection"]["tempo"], 0.7)
            self.assertEqual(reloaded_meta["stale_edit_ignored"], False)

    def test_second_browser_cannot_overwrite_a_newer_saved_edit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            task = {"id": 112, "session_id": 171}
            write_json(
                workspace / "SessionOutput/storyboard/srt_storyboard.json",
                source_storyboard("04_02_StoryBoard", "原始文案"),
            )
            services = self.make_services(workspace)
            initial, _meta = services.load_plan(task, sc=services)
            browser_a = deepcopy(initial)
            browser_b = deepcopy(initial)
            browser_a["shots"][0]["scenes"][0]["dialogues"][0]["text"] = "标签 A 的编辑"
            services.save_edit_and_source_storyboard(task, workspace, browser_a, sc=services)
            browser_b["shots"][0]["scenes"][0]["dialogues"][0]["text"] = "标签 B 的旧页面"

            with self.assertRaises(HTTPException) as error:
                services.save_edit_and_source_storyboard(task, workspace, browser_b, sc=services)

            self.assertEqual(error.exception.status_code, 409)
            self.assertEqual(error.exception.detail["code"], "storyboard_edit_changed")
            saved = read_json(workspace / "SessionOutput/storyboard/koubo_storyboard_edit.json")
            self.assertEqual(saved["shots"][0]["scenes"][0]["dialogues"][0]["text"], "标签 A 的编辑")

    def test_source_only_binding_change_invalidates_saved_edit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            task = {"id": 112, "session_id": 171}
            source = source_storyboard("04_02_StoryBoard", "原始文案")
            ...
            write_json(workspace / "SessionOutput/storyboard/srt_storyboard.json", source)
            services = self.make_services(workspace)
            plan = edit_storyboard("04_02_StoryBoard", "编辑文案", source_hash)
            services.save_edit_and_source_storyboard(task, workspace, plan, sc=services)
            projected = read_json(workspace / "SessionOutput/storyboard/srt_storyboard.json")
            projected_dialogue = projected["shots"][0]["scenes"][0]["dialogue_items"][0]
            projected_dialogue["working_assets"]["video"] = {
                "slot": "Video_Final",
                "source_type": "generated",
                "path": "SessionOutput/storyboard/Working/srt_0001_Video_Final.mp4",
            }
            write_json(workspace / "SessionOutput/storyboard/srt_storyboard.json", projected)

            loaded, meta = services.load_plan(task, sc=services)

            loaded_video = loaded["shots"][0]["scenes"][0]["dialogues"][0]["working_assets"]["video"]
            self.assertEqual(meta["stale_edit_ignored"], True)
            self.assertEqual(loaded_video["path"], "SessionOutput/storyboard/Working/srt_0001_Video_Final.mp4")
    ...
    def test_save_does_not_project_new_image_into_source_image_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            source = source_storyboard("04_02_StoryBoard", "无原图但已有新图")
            source_dialogue = source["shots"][0]["scenes"][0]["dialogue_items"][0]
            source_dialogue["image_path"] = ""
            source_hash = storyboard_plan_services.stable_storyboard_sha256(source)
            write_json(workspace / "SessionOutput/storyboard/srt_storyboard.json", source)
            services = self.make_services(workspace)
            plan = edit_storyboard("04_02_StoryBoard", "无原图但已有新图", source_hash)
            dialogue = plan["shots"][0]["scenes"][0]["dialogues"][0]
            new_image = "SessionOutput/storyboard/Working/dak_0002_Image_New.png"
            dialogue["source_image_paths"] = []
            dialogue["image_path"] = ""
            dialogue["bound_image_path"] = new_image
            dialogue["working_assets"] = {
                "audio": {"slot": "Audio_Final", "source_type": "", "path": ""},
                "images": [
                    {"slot": "Image_New", "source_type": "history", "path": new_image},
                    {"slot": "Image_02", "source_type": "", "path": ""},
                ],
                "video": {"slot": "Video_Final", "source_type": "", "path": ""},
            }

            services.save_edit_and_source_storyboard({"id": 112, "session_id": 171}, workspace, plan, sc=services)

            projected = read_json(workspace / "SessionOutput/storyboard/srt_storyboard.json")
            projected_dialogue = projected["shots"][0]["scenes"][0]["dialogue_items"][0]
            self.assertEqual(projected_dialogue.get("image_path", ""), "")
            self.assertEqual(projected_dialogue.get("key_frame_paths", []), [])
            self.assertEqual(projected_dialogue.get("bound_image_path", ""), new_image)
            self.assertEqual(projected_dialogue["working_assets"]["images"][0]["path"], new_image)

    def test_save_restores_customer_hidden_tts_runtime_fields_from_trusted_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            source = source_storyboard("04_02_StoryBoard", "音色配置回填")
            source_hash = storyboard_plan_services.stable_storyboard_sha256(source)
            write_json(workspace / "SessionOutput/storyboard/srt_storyboard.json", source)
            trusted_candidate = {
                "candidate_id": "tts_001",
            ...
            plan = edit_storyboard("04_02_StoryBoard", "音色配置回填", source_hash)
            plan["storyboard_tts_selection"] = {
                **customer_candidate,
                "provider": "",
                "model": "",
            ...
            saved = read_json(workspace / "SessionOutput/storyboard/koubo_storyboard_edit.json")
            selection = saved["storyboard_tts_selection"]
            self.assertEqual(selection["provider"], "trusted-provider")
            self.assertEqual(selection["model"], "trusted-model")
            self.assertEqual(selection["top_candidates"][0]["provider"], "trusted-provider")
            self.assertEqual(selection["top_candidates"][0]["model"], "trusted-model")

    def test_normalize_source_plan_treats_existing_new_image_pollution_as_bound_image(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            new_image = "SessionOutput/storyboard/Working/dak_0002_Image_New.png"
            source = source_storyboard("04_02_StoryBoard", "旧脏数据")
            source_dialogue = source["shots"][0]["scenes"][0]["dialogue_items"][0]
            source_dialogue["image_path"] = new_image
            source_dialogue["bound_image_path"] = new_image
            source_dialogue["working_assets"] = {
                "audio": {"slot": "Audio_Final", "source_type": "", "path": ""},
                "images": [
                    {"slot": "Image_New", "source_type": "history", "path": new_image},
                    {"slot": "Image_02", "source_type": "", "path": ""},
            ...
            dialogue = plan["shots"][0]["scenes"][0]["dialogues"][0]
            self.assertEqual(meta["has_saved_edit"], False)
            self.assertEqual(dialogue.get("source_image_paths", []), [])
            self.assertEqual(dialogue.get("image_path", ""), "")
            self.assertEqual(dialogue.get("bound_image_path", ""), new_image)
            self.assertEqual(dialogue["working_assets"]["images"][0]["path"], new_image)
    ...
    def test_storyboard_video_delete_archives_video_only_raw_tail_and_clears_execution_steps(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            archived = services.archive_removed_generated_assets(workspace, previous_plan, next_plan, "storyboard_save_removed_generated_asset", sc=services)
    ...
    def test_clear_new_image_archives_standard_image_slot_orphans(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plan, old_path, deleted = services.clear_asset_from_plan(workspace, plan, "scene_001_dialogue_003", "image", sc=services)
